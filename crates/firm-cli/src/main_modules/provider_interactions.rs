...
pub(super) fn work_contract_prompt(
    ...
    format!(
        "You are {name}, the {role} member of Agent Team run \"{objective}\".\n\
         ...
         - Send a canonical Work-linked Message through the authenticated Member Role Action with: \"$HARNESS_BIN\" member message send --recipient-agent-id <stable-agent-identity> --work-id <discussed-work-id> --body '<markdown>'. For ordinary member progress, `<discussed-work-id>` is {work_id}; for Host assignment/progress/retry, it is `<recipient-work-id-from-board>`. The bound command derives your sender identity and live runtime scope; never select a sender identity.\n\
