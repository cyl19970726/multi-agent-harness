...
try{
  ...
  const makePage=async token=>{const next=await browser.newPage({viewport:{width:1440,height:1000}});next.on("console",message=>{if(message.type()==="error")consoleErrors.push(message.text());});next.on("pageerror",error=>consoleErrors.push(error.message));next.on("response",response=>{if(response.status()>=400)httpFailures.push(`${response.status()} ${response.url()}`);});await next.addInitScript(({token,fixture})=>{window.__AGENTFIRM_BOOTSTRAP__={capabilityToken:token};class QuietEventSource{addEventListener(type,listener){if(fixture&&type==="snapshot")queueMicrotask(()=>listener({data:JSON.stringify({generated_at:"2026-08-12T08:10:00Z",execution_space_id:"fixture-space"})}));}close(){}}Object.defineProperty(window,"EventSource",{value:QuietEventSource,configurable:true});},{token,fixture:!liveConfig});if(!liveConfig)await next.route("**/v1/**",async route=>{
    ...
    if(url.pathname==="/v1/events"){
      ...
      const scope=token==="fixture-host-token"?{execution_space_id:"fixture-space",project_id:"fixture-project",team_run_id:team.latest_run.id,member_run_id:"member-run-host",agent_session_id:"host-thread-current",agent_session_generation:1}:{execution_space_id:"fixture-space",project_id:"fixture-project",team_run_id:team.latest_run.id,member_run_id:"member-run-mira",agent_session_id:"session-mira-current",agent_session_generation:3};
  ...
  if(!liveConfig){await page.getByText(/Implemented the owner-bound Session projection/).waitFor();await page.locator(".aw-native-facts-trail > summary").first().waitFor();}
  if(!liveConfig){
  ...
  if(!liveConfig){
    const factsTrail=page.locator(".aw-native-facts-trail",{has:page.getByText("Validated owner-bound native Session",{exact:true})}).first();
    await factsTrail.locator("> summary").click();await page.getByText("Validated owner-bound native Session",{exact:true}).waitFor();
