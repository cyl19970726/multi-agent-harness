...
pub(crate) const REVIEWED_CLAUDE_CODE_VERSION: &str = "2.1.220";
pub(crate) const REVIEWED_CLAUDE_AGENT_SDK_VERSION: &str = "0.3.220";
...
#[derive(Debug, Clone)]
pub(crate) struct ClaudeTeamRuntimeConfig {
...
pub(crate) struct ClaudeTeamRuntime {
...
impl ClaudeTeamRuntime {
    ...
    fn contract_observation(&self) -> crate::runtime_adapter_contract::RuntimeObservation {
        let session = self
            .authority_session
            .as_ref()
    ...
    pub(crate) fn close_owned_runtime(&mut self, reason: &str) -> CliResult<ClaudeCloseEvidence> {
...
pub(crate) struct ClaudeCloseEvidence {
...
impl TeamRuntimeAdapter for ClaudeTeamRuntime {
    ...
    fn project_live(
        ...
        if tools > 0 {
            ...
            Some((
                crate::provider_event_api::LiveProviderActivityKind::ResponseStreaming,
                format!(
...
mod tests {
    ...
    fn runner_events_are_deny_unknown_at_the_envelope_boundary() {
        let event =
            RunnerEvent::parse(r#"{"event":"session_bound","data":{"sessionId":"native-1"}}"#)
                .unwrap();
