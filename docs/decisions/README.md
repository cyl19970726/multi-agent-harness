...
| [0076](0076-one-closed-table-for-how-an-execution-cycle-ends.md) | accepted; Owner 2026-09-15 (core 6) | `CycleEnding` is the one closed table for how an ExecutionCycle ends: twelve variants cover the `Ok` outcome and every `Err` path the five adapters take, with a stated precedence order, a small closed `ProviderFailureCode` that keeps the provider's own text as `detail`, one fixed `member_actions.action_type` table, a `provider_status` populated on all five providers through a shared `take_cycle_ending`, and an additive `ending` on the cycle correlation; a requested Interrupt/Close that races a normal terminal is settled by that terminal instead of being dropped on all five providers, an empty terminal is `EmptyOutput` everywhere and counts toward the unproductive-round breaker, and once the input has crossed the provider boundary no ending may claim replay-safety |
