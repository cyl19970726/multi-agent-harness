...
| Codex | [codex.md](codex.md) | adapter implemented; installed `0.148.0-alpha.9` current for the reviewed host-driven slices | `codex_app_server` is the only new Codex Team mode; DEV-26 proved exact-thread open/resume, completed rounds, current-turn interrupt, narrow Close, and Reopen. Live steer and provider-driven Goal supervision remain review-required capability slices; bounded `codex_exec` belongs to Workflow and historical reads. |
