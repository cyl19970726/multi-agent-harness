...
| S4 `ExecutionCycle` | Supervisor wake/claim → drive → settle; durable `ProviderCycleCorrelation` including its closed `ending` | `TeamRuntimeAdapter::run_cycle`: input-acceptance receipt, control injection, terminal boundary, `ExecutionCycleOutcome`; `CycleTimeouts` separates input acceptance, transport liveness and control settlement; typed `CycleSettlement` preserves receipt and interruption evidence; `CycleEnding` is the closed table of how a cycle ends and keys both `member_actions.action_type` and `provider_status`, with each adapter mapping its own `Err` endings through one wildcard-free match and draining them via `take_cycle_ending` (DEV-156, ADR 0076; `firm-runtime-contract/src/{cycle,cycle_ending,timeouts}.rs`) | one native turn / prompt / query / followup and its terminal frame |
