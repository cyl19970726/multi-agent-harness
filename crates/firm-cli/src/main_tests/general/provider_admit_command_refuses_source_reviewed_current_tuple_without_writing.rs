...
fn provider_admit_command_refuses_source_reviewed_current_tuple_without_writing() {
    ...
    let store = HarnessStore::new(&root).with_provider_compatibility_scope(project_id, &store_id);
    ...
    let error =
        provider_admit_command_with_probe(&store, &resolved, &args, |_| Ok(version.to_string()))
