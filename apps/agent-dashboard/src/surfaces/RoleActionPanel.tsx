...
const CRITICAL = new Set(["accept_work", "cancel_work", "reconcile_message_delivery", "resolve_runtime_recovery", "close_member_run", "retire_member_run", "cleanup_workspace", "waive_gate", "revoke_waiver", "start_daemon", "stop_daemon", "recover_daemon_predecessor"]);
const EXECUTABLE = new Set([
  "create_work", "assign_work", "release_work", "accept_work",
...
const FIELD_SPECS: Record<string, Array<{ name: string; label: string; multiline?: boolean }>> = {
  ...
  assign_work: [{ name: "membership_id", label: "TeamMembership ID" }],
  ...
  recover_daemon_predecessor: [{ name: "evidence_ref", label: "Process and provider-group termination evidence ref" }],
  ...
  interrupt_member_run: [{name:"reason",label:"Interrupt reason",multiline:true}],
