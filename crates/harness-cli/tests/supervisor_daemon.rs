...
//!
//! Note: when `HARNESS_MEMBER_SUPERVISOR_TEST_IDLE_MS` is set, `team-run start`
...
/// `team_run_start.rs` but with NO `HARNESS_MEMBER_SUPERVISOR_TEST_IDLE_MS`
...
fn run_with_fake_kimi_daemon(
    ...
    std::process::Command::new(env!("CARGO_BIN_EXE_harness"))
        ...
        // Deliberately NOT setting HARNESS_MEMBER_SUPERVISOR_TEST_IDLE_MS — we
        ...
        .env("HARNESS_DAEMON_SPAWN_TIMEOUT_SECS", "15")
...
fn generate_plist_is_valid_xml() {
    ...
    let plist_path = agents_dir.join("com.harness.supervisor.test-run-123.plist");
    assert!(
        plist_path.exists(),
        "plist not found at {}",
        plist_path.display()
