...
pub fn companies_dir(firm_home: &Path) -> PathBuf {
    firm_home.join("companies")
}

pub fn company_store_root(firm_home: &Path, id: &str) -> PathBuf {
    companies_dir(firm_home).join(id)
}

pub fn registry_path(firm_home: &Path) -> PathBuf {
    companies_dir(firm_home).join("registry.json")
}

pub fn active_company_path(firm_home: &Path) -> PathBuf {
    firm_home.join("ACTIVE_COMPANY")
}
...
impl CompanyRegistry {
    pub fn load(firm_home: &Path) -> CompanyStoreResult<Self> {
    ...
    pub fn save(&mut self, firm_home: &Path) -> CompanyStoreResult<()> {
        self.format_version = REGISTRY_FORMAT_VERSION;
        std::fs::create_dir_all(companies_dir(firm_home))?;
        std::fs::write(
...
pub fn context_for_id(firm_home: &Path, id: &str) -> CompanyStoreResult<Option<CompanyContext>> {
    validate_company_id(id)?;
    let registry = CompanyRegistry::load(firm_home)?;
    ...
    let store_root = company_store_root(firm_home, id);
...
pub fn register_and_activate(
    ...
    let ctx = CompanyContext {
        id: id.to_string(),
        name: name.to_string(),
        store_root: company_store_root(firm_home, id),
    };
    write_metadata(&ctx)?;
    let mut registry = CompanyRegistry::load(firm_home)?;
    registry.upsert(
    ...
    write_active_company(firm_home, &ctx.id)?;
    Ok(ctx)
}
...
pub fn active_company_id(firm_home: &Path) -> CompanyStoreResult<Option<String>> {
    let registry = CompanyRegistry::load(firm_home)?;
    if let Some(id) = registry.current_company_id {
        return Ok(Some(id));
    }
    read_active_company(firm_home)
}

pub fn list_companies(firm_home: &Path) -> CompanyStoreResult<Vec<CompanyContext>> {
    let mut out = Vec::new();
    ...
    let companies = companies_dir(firm_home);
...
pub fn write_active_company(firm_home: &Path, id: &str) -> CompanyStoreResult<()> {
    std::fs::create_dir_all(firm_home)?;
    std::fs::write(active_company_path(firm_home), format!("{id}\n"))?;
    Ok(())
}

pub fn read_active_company(firm_home: &Path) -> CompanyStoreResult<Option<String>> {
    match std::fs::read_to_string(active_company_path(firm_home)) {
        Ok(text) => {
...
pub fn firm_home() -> CompanyStoreResult<PathBuf> {
    project::firm_home().map_err(|e| match e {
