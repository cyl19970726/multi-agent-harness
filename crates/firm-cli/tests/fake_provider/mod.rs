...
use crate::firm_env::clear_inherited_native_firm_env;
...
if [ -n "${FAKE_KIMI_ENV_MARKER:-}" ]; then
  env | grep '^FIRM_' | sort > "$FAKE_KIMI_ENV_MARKER"
fi
if [ "$1" != "acp" ]; then
  ...
  case "$line" in
    ...
    *'"method":"session/prompt"'*)
      ...
      if [ "${FAKE_KIMI_KEEP_WORK_ACTIVE:-0}" = "1" ] && [ "$prompt_count" = "1" ]; then
        ...
        sleep 0.1
        work_json=$("$FIRM_BIN" --project "$FIRM_PROJECT_ID" team-run work list \
      ...
      if [ -n "${FAKE_KIMI_PEER_ACK_CONFIG:-}" ] && [ -s "${FAKE_KIMI_PEER_ACK_CONFIG}" ] && [ "$prompt_count" = "2" ]; then
        ...
        if [ "${FIRM_MEMBER_RUN_ID:-}" = "$ack_from" ]; then
          sleep 0.1
          work_id=$("$FIRM_BIN" --project "$FIRM_PROJECT_ID" team-run work list \
            --team-run-id "$FIRM_TEAM_RUN_ID" \
            --member-run-id "$FIRM_MEMBER_RUN_ID" \
            | sed -n 's/.*"id": "\([^"]*\)".*/\1/p' | sed -n '1p')
          "$FIRM_BIN" --project "$FIRM_PROJECT_ID" team-run send \
            --id "$FIRM_TEAM_RUN_ID" \
            --from "$FIRM_MEMBER_RUN_ID" \
            --to "$ack_to" \
            --kind message \
      ...
      if [ "${FAKE_KIMI_MESSAGE_DURING_TURN:-0}" = "1" ]; then
        ...
        # ACP frame and publish its canonical WorkDelivery receipt before this bound
...
if [ -n "${FAKE_CLAUDE_ENV_MARKER:-}" ]; then
  env | grep '^FIRM_' | sort > "$FAKE_CLAUDE_ENV_MARKER"
fi
if [ "$1" = "--version" ]; then
  printf '%s\n' '2.1.181 (Claude Code)'
  exit 0
...
///
/// `envs` are the base env pairs (HOME / FIRM_HOME from `TempHome::envs()`);
...
impl DeliveryDriver {
    pub fn new(
        ...
        Self {
            bin: PathBuf::from(env!("CARGO_BIN_EXE_firm")),
...
for line in sys.stdin:
    ...
    if t == 'get_state':
        ...
        if prompt_count == 1 and os.environ.get('FAKE_PI_SUBMIT_WORK') == '1':
            ...
            member_run = os.environ['FIRM_MEMBER_RUN_ID']
