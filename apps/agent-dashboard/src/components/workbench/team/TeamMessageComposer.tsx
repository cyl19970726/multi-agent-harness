...
export function TeamMessageComposer({ actions, members, works, replyTo, teamId, teamRunId, actionsCurrent, onAction, onClearReply, onCompleted, collapsibleOnMobile=false, fixedRecipient, variant="panel" }: {
  ...
  if (variant === "conversation") return <section aria-labelledby="team-composer-title" className="border-t border-border bg-background/95 px-3 py-3 backdrop-blur"><div className="agent-team-composer mx-auto max-w-4xl rounded-xl p-2.5"><div className="flex min-w-0 flex-wrap items-center gap-2 border-b border-border px-1 pb-2"><div className="flex min-w-0 items-center gap-2 rounded-md bg-secondary px-2.5 py-1.5"><span className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">To</span><span className="truncate text-xs font-semibold">{fixedRecipient?.label ?? "Team"}</span></div><label className="flex min-w-0 flex-1 items-center gap-2 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground"><span className="shrink-0">Related Work</span><select value={workId} onChange={(event) => setWorkId(event.target.value)} className="h-8 min-w-0 flex-1 rounded-md border border-border bg-card px-2 text-xs font-normal normal-case tracking-normal"><option value="">No Work link</option>{works.map((work) => <option key={work.work_id} value={work.work_id}>{work.title || work.work_id}</option>)}</select></label><label className="flex items-center gap-1.5 text-[10px] text-muted-foreground"><input type="checkbox" checked={responseRequired} onChange={(event) => setResponseRequired(event.target.checked)}/>Response required</label>{replyTo && <Button size="sm" variant="secondary" onClick={onClearReply}>Cancel reply</Button>}</div>{replyTo && <p className="mt-2 truncate rounded-md bg-primary/[0.05] px-2 py-1 text-[9px] text-muted-foreground">Correlated reply to {replyTo.sender.id} · {replyTo.message_id}</p>}<div className="mt-2 flex min-w-0 items-end gap-2"><textarea aria-label="Message" value={body} onChange={(event) => setBody(event.target.value)} className="min-h-14 min-w-0 flex-1 resize-y border-0 bg-transparent p-2 text-sm outline-none placeholder:text-muted-foreground/70" rows={1} placeholder={`Write a message${fixedRecipient ? ` to ${fixedRecipient.label}` : ""}…`}/><Button size="sm" className="min-h-11 shrink-0 px-4" disabled={!actionsCurrent || busy || !recipientIds || !body.trim() || Boolean(action?.disabled_reason)} title={!actionsCurrent ? "Awaiting a current authoritative view" : action?.disabled_reason ?? undefined} onClick={execute}><SendHorizontal className="size-3.5"/>{busy ? "Sending…" : replyTo ? "Reply" : "Send"}</Button></div><div className="flex items-center justify-between border-t border-border px-1 pt-2"><p className="text-[9px] text-muted-foreground">Authenticated ordinary Message · never current-turn Steer</p><p className="hidden text-[9px] text-muted-foreground sm:block">Shift + Enter for a new line</p></div>{status && <p role="status" className="mt-1 text-[10px] text-muted-foreground">{status}</p>}</div></section>;
