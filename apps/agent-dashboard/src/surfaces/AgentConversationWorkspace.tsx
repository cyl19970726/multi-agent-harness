...
import {
  ...
  Command, Filter, Inbox, MessageSquare,
...
export function AgentConversationWorkspace({
  ...
  return <Tooltip.Provider delayDuration={350}>
    <main className="agent-team-surface agent-workspace h-full min-h-0 flex-1 overflow-hidden" data-testid="agent-workspace">
      <div className="grid h-full min-h-0 grid-cols-1 lg:grid-cols-[18.75rem_minmax(40rem,1fr)_19.625rem]">
        ...
        <section className="agent-workspace-center flex min-h-0 min-w-0 flex-col">
          <header className="agent-workspace-header flex min-h-[5.65rem] shrink-0 items-center gap-3 border-b border-border px-4 sm:px-6">
            ...
            <button type="button" className="group flex min-w-0 flex-1 items-center gap-3 text-left" onClick={()=>setProfileOpen(true)} aria-label={`Open ${selected.display_name} configuration`}>
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 flex min-w-0 items-center gap-2 text-[11px] text-muted-foreground"><span className="truncate">{selected.role}</span><span>·</span><span>{selected.provider ?? "provider not bound"}</span><span>·</span><span className="truncate">{data.selected_session_id ?? "no current Session"}</span></span>
              </span>
            </button>
            <div className="hidden items-center gap-2 md:flex"><RuntimePill state={selected.runtime_status}/></div>
          ...
          <Tabs.Root value={mode} onValueChange={value=>{setContextSelection(null);onSelectionChange({agentWorkspaceMode:value as WorkspaceMode});}} className="flex min-h-0 flex-1 flex-col">
            <div className="flex min-h-12 shrink-0 items-end border-b border-border px-4 sm:px-6">
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-transparent">
    ...
    <ScrollArea.Root className="min-h-0 flex-1 overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="px-2 pb-5">
      {visible.map((agent,index)=><div key={agent.agent_member_ref.id}>{index===0&&<p className="px-3 pb-1 pt-2 text-[9px] font-semibold uppercase tracking-[.15em] text-muted-foreground">Host Agent</p>}{index===1&&<p className="px-3 pb-1 pt-5 text-[9px] font-semibold uppercase tracking-[.15em] text-muted-foreground">Team Members</p>}<button type="button" onClick={()=>onSelect(agent)} data-selected={agent.agent_member_ref.id===selectedId} className="agent-roster-row group flex w-full items-center gap-3 px-3 py-3 text-left">
        ...
        <span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="truncate text-[13px] font-semibold">{agent.display_name}</span>{agent.is_host&&<span className="text-[9px] font-semibold text-primary">Host</span>}</span><span className="mt-0.5 block truncate text-[10px] text-muted-foreground">{agent.role} · {agent.runtime_state??agent.capacity}</span></span>
        <span className="flex flex-col items-end gap-1"><Circle className={`size-2 fill-current ${agent.runtime_state==="running"?"text-status-running":agent.capacity==="available"?"text-status-good":"text-muted-foreground/35"}`}/>{(agent.queued_work_count??0)>0&&<span className="text-[9px] text-primary">{agent.queued_work_count}</span>}</span>
      </button></div>)}
...
function SessionCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(next:ContextSelection)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="agent-session-stream w-full px-5 pb-8 pt-2 sm:px-7">
    {rows.length?rows.map(row=>row.kind==="message"?<AuthoredTurn key={`message:${row.message.message_id}`} data={data} message={row.message} selectedAgentId={data.selected_agent.agent_member_ref.id} onSelect={()=>onSelect({kind:"message",message:row.message})}/>:<ExpandableEvent key={row.event.event_id} data={data} event={row.event} onSelect={()=>onSelect({kind:"event",event:row.event})}/>):<EmptyCanvas title="No Session activity yet" detail={data.session_activity.disabled_reason??"The selected provider Session has not emitted a display-safe event."}/>}
...
function ExpandableEvent({data,event,onSelect}:{data:AgentWorkspaceData;event:AgentWorkspaceActivityItem;onSelect:()=>void}){
  ...
  return <article className="agent-event-row border-b border-border/65" data-open={open}>
    <button type="button" className="flex min-h-10 w-full items-center gap-2.5 py-2 text-left" aria-expanded={open} onClick={()=>{setOpen(value=>!value);onSelect();}}>{open?<ChevronDown className="size-3 text-muted-foreground"/>:<ChevronRight className="size-3 text-muted-foreground"/>}<span className="grid size-6 place-items-center rounded-md bg-secondary text-muted-foreground"><Icon className="size-3"/></span><span className="min-w-0 flex-1 truncate text-[11px] font-medium">{event.title}</span><span className="text-[9px] text-muted-foreground">{event.status}</span><span className="w-16 text-right text-[9px] text-muted-foreground">{formatTime(event.occurred_at)}</span></button>
...
function MessagesCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(next:ContextSelection)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="mx-auto max-w-[58rem] px-5 pb-10 pt-5 sm:px-8">
    <div className="agent-message-toolbar mb-4 flex flex-wrap items-center gap-2 border-b border-border pb-4"><div className="flex rounded-lg bg-secondary/70 p-1">{(["all","inbox","outbox","unread"] as const).map(item=><button key={item} type="button" data-active={lens===item} onClick={()=>setLens(item)} className="rounded-md px-3 py-1.5 text-[10px] font-semibold capitalize text-muted-foreground data-[active=true]:bg-background data-[active=true]:text-foreground data-[active=true]:shadow-sm">{item}</button>)}</div><label className="relative ml-auto min-w-[14rem] flex-1 sm:max-w-[20rem]"><Search className="pointer-events-none absolute left-3 top-2.5 size-3.5 text-muted-foreground"/><span className="sr-only">Search messages</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search messages" className="h-9 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-xs"/></label><Button size="icon" variant="secondary" aria-label="Message filters"><Filter className="size-3.5"/></Button></div>
    {visible.length?<div className="divide-y divide-border/75">{visible.map(message=><button key={message.message_id} type="button" className="agent-message-row grid w-full grid-cols-[2rem_minmax(0,1fr)_8rem] gap-3 py-4 text-left" onClick={()=>onSelect({kind:"message",message})}><span className="grid size-8 place-items-center rounded-full bg-secondary"><MessageSquare className="size-3.5"/></span><span className="min-w-0"><span className="flex items-center gap-2"><b className="truncate text-[11px]">{message.sender.id===selectedId?"Outbox":message.sender.id}</b><span className="text-[9px] text-muted-foreground">{message.kind}</span>{message.work_id&&<Badge tone="muted">{shortId(message.work_id)}</Badge>}</span><span className="mt-1 line-clamp-2 text-[12px] leading-5 text-foreground/85">{message.body}</span></span><span className="text-right text-[9px] text-muted-foreground"><span className="block">{formatTime(message.created_at)}</span><span className="mt-2 block">{message.deliveries.map(delivery=>delivery.status).join(", ")||"recorded"}</span></span></button>)}</div>:<EmptyCanvas title="No matching messages" detail="Inbox and Outbox stay in Harness coordination records; provider-private transcripts are not mirrored here."/>}
...
function WorkCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(work:WorkSummary)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="mx-auto max-w-[60rem] px-5 pb-10 pt-5 sm:px-8">
    <div className="mb-5 flex flex-wrap items-center gap-5 border-b border-border pb-3">{(["current","open","active","review","closed","eligible"] as const).map(item=><button key={item} type="button" data-active={lens===item} onClick={()=>setLens(item)} className="agent-work-lens relative pb-2 text-[10px] font-semibold capitalize text-muted-foreground data-[active=true]:text-foreground">{item}</button>)}<span className="ml-auto text-[10px] text-muted-foreground">{visible.length} responsibility records</span></div>
    {visible.length?<div className="space-y-0">{visible.map(work=><button key={work.work_id} type="button" className="agent-work-row grid w-full grid-cols-[minmax(0,1fr)_7rem_7rem] gap-4 border-b border-border/75 py-4 text-left" onClick={()=>onSelect(work)}><span className="min-w-0"><span className="flex items-center gap-2"><span className="truncate text-[13px] font-semibold">{work.title||work.work_id}</span>{work.condition!=="normal"&&<Badge tone="bad">{work.condition}</Badge>}</span><span className="mt-1 block truncate text-[10px] text-muted-foreground">{shortId(work.work_id)} · v{work.work_revision} · {work.priority}</span><span className="mt-2 line-clamp-2 text-[11px] leading-5 text-foreground/72">{work.completion_criteria_markdown}</span></span><span><span className="agent-team-phase-label">{work.phase}</span><span className="mt-2 block text-[9px] text-muted-foreground">{work.delivery_summary.recovery_class||"delivery clear"}</span></span><span className="text-right"><span className="text-[10px] font-semibold">{work.gate_summary.passed}/{work.gate_summary.required} gates</span><span className="mt-2 block text-[9px] text-muted-foreground">{formatTime(work.updated_at)}</span></span></button>)}</div>:<EmptyCanvas title="No Work in this view" detail="Eligibility is not ownership. Work remains authoritative in the Team Work kernel."/>}
  </div></ScrollArea.Viewport></ScrollArea.Root>;
}
...
function AgentContextRail({view,data,selected,currentWork,actions}:{view:RoleView<AgentWorkspaceData>;data:AgentWorkspaceData;selected:ContextSelection;currentWork?:WorkSummary;actions:AllowedAction[]}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="px-5 pb-8 pt-5">
    ...
    {selfPrivate?<ContextSection title={isHost?"Host Session":"Native Session"}><ContextFact label="State" value={data.selected_agent.runtime_status??"unknown"}/><ContextFact label="Provider" value={data.selected_agent.provider??"not bound"}/><ContextFact label="Session" value={shortId(data.selected_session_id)}/><ContextFact label="Last activity" value={formatTime(data.context_summary.last_activity_at)}/></ContextSection>:<ContextSection title="Public Agent context"><p className="text-[11px] leading-5 text-muted-foreground">Provider-private Session, runtime and workspace facts are owner-bound and are not part of this Host view.</p></ContextSection>}
    {isHost&&<ContextSection title="Team Inbox"><div className="mb-2 flex items-baseline justify-between"><span className="text-[10px] text-muted-foreground">Needs Host</span><b className="text-[12px] text-primary">{data.context_summary.unread_count}</b></div>{incoming.slice(0,3).map(message=><ContextMessageRow key={message.message_id} data={data} message={message}/>)}{!incoming.length&&<p className="text-[10px] text-muted-foreground">No authored message currently needs Host attention.</p>}</ContextSection>}
    {isHost&&<ContextSection title="Assigned Work"><div className="mb-2 flex items-baseline justify-between"><span className="text-[10px] text-muted-foreground">Current responsibility</span><b className="text-[10px]">{activeWorks.length} active</b></div>{activeWorks.slice(0,3).map(work=><ContextWorkRow key={work.work_id} work={work}/>)}</ContextSection>}
    <ContextSection title="Latest Review & Evidence">{evidenceWorks.length?evidenceWorks.slice(0,2).map(work=><ContextWorkRow key={work.work_id} work={work} evidence/>):<p className="text-[10px] leading-5 text-muted-foreground">No review, evidence, finding or failure is currently projected.</p>}</ContextSection>
    <ContextSection title={isHost?"Available Host Controls":"Available Controls"}><p className="text-[10px] leading-5 text-muted-foreground">{data.context_summary.authorization_count} canonical actions are currently executable.</p>{actions.slice(0,6).map(action=><p key={`${action.kind}:${action.target_ref.id}`} className="mt-2 flex items-center gap-2 text-[10px]"><Circle className={`size-2 fill-current ${action.disabled_reason?"text-muted-foreground/40":"text-status-good"}`}/><span className="truncate">{actionLabel(action.kind)}</span></p>)}</ContextSection>
    <ContextSection title="Projection"><ViewProvenance view={view}/></ContextSection>
  </div></ScrollArea.Viewport><ScrollArea.Scrollbar orientation="vertical" className="flex w-2 p-0.5"><ScrollArea.Thumb className="rounded-full bg-border"/></ScrollArea.Scrollbar></ScrollArea.Root>;
}

function WorkContext({title="Current Work",work}:{title?:string;work?:WorkSummary}){return <ContextSection title={title} primary>{work?<><h3 className="text-[13px] font-semibold leading-5">{work.title||work.work_id}</h3><p className="mt-1 text-[10px] text-muted-foreground">{shortId(work.work_id)} · v{work.work_revision}</p><div className="mt-4"><ContextFact label="State" value={`${work.phase} · ${work.condition}`}/><ContextFact label="Priority" value={work.priority}/><ContextFact label="Delivery" value={String(work.delivery_summary.recovery_class??"clear")}/><ContextFact label="Gates" value={`${work.gate_summary.passed}/${work.gate_summary.required}`}/></div></>:<p className="text-[11px] leading-5 text-muted-foreground">No Work is currently owned or eligible for this Agent.</p>}</ContextSection>}
function MessageContext({message}:{message:MessageSummary}){return <ContextSection title="Selected Message" primary><p className="text-[12px] font-semibold">{message.sender.id}</p><p className="mt-2 line-clamp-4 text-[11px] leading-5 text-muted-foreground">{message.body}</p><div className="mt-4"><ContextFact label="Delivery" value={message.deliveries.map(item=>item.status).join(", ")||"recorded"}/><ContextFact label="Linked Work" value={shortId(message.work_id)}/></div></ContextSection>}
...
function AgentComposer({data,actions,actionsCurrent,selectedRunId,onAction,onCompleted}:{data:AgentWorkspaceData;actions:AllowedAction[];actionsCurrent:boolean;selectedRunId:string|null;onAction:RoleActionExecutor;onCompleted:()=>void}){
  ...
  const actionControl=<label className="flex min-w-0 items-center gap-2 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground"><SlidersHorizontal className="size-3 shrink-0 text-primary"/><span className="sr-only">Action</span><select aria-label="Composer action" value={selected?keyForAction(selected):""} onChange={event=>setSelectedKey(event.target.value)} className="h-8 min-w-0 w-full rounded-md border border-border bg-background px-2 text-[10px] font-medium normal-case tracking-normal text-foreground"><option value="" disabled>No action authorized</option>{usable.map(action=><option key={keyForAction(action)} value={keyForAction(action)} disabled={Boolean(action.disabled_reason)}>{actionLabel(action.kind)}{action.disabled_reason?` — ${action.disabled_reason}`:""}</option>)}</select></label>;
...
function ProfileDialog({data,onClose,closeRef}:{data:AgentWorkspaceData;onClose:()=>void;closeRef:React.RefObject<HTMLButtonElement>}){
  ...
  return <div className="fixed inset-0 z-50 bg-[#241b16]/25 backdrop-blur-[2px]" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section role="dialog" aria-modal="true" aria-label={`${selected.display_name} configuration`} className="agent-profile-drawer absolute inset-y-0 right-0 w-[min(92vw,33rem)] overflow-y-auto border-l border-border bg-background shadow-2xl">
    <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-5 py-4 backdrop-blur"><Avatar name={selected.display_name} identity={selected.agent_member_ref.id} size="lg" tone="running"/><div className="min-w-0 flex-1"><h2 className="truncate font-serif text-xl">{selected.display_name}</h2><p className="mt-0.5 text-[10px] text-muted-foreground">{selected.role} · durable AgentMember</p></div><Button ref={closeRef} size="icon" variant="secondary" onClick={onClose} aria-label="Close Agent configuration"><X className="size-4"/></Button></header>
    <div className="space-y-7 px-6 py-6"><ProfileSection title="Identity"><ContextFact label="AgentMember" value={selected.agent_member_ref.id}/><ContextFact label="Role" value={selected.role}/><ContextFact label="Organization" value={selected.organization_status}/>{c.description&&<p className="mt-3 text-[12px] leading-5 text-muted-foreground">{c.description}</p>}</ProfileSection>
      ...
      <ProfileSection title="Skills and capabilities"><TagList values={c.skill_refs} empty="No Skill refs are projected."/><TagList values={c.capabilities} empty="No capabilities are projected."/></ProfileSection>
      <ProfileSection title="Prompt and restrictions"><ContextFact label="Prompt ref" value={c.prompt_ref??c.prompt_projection}/><ContextFact label="Forbidden actions" value={c.forbidden_actions.length?c.forbidden_actions.join(", "):c.forbidden_actions_projection}/><p className="mt-3 text-[10px] leading-5 text-muted-foreground">Unavailable fields stay explicit; the browser does not infer configuration from provider behavior.</p></ProfileSection>
      <ProfileSection title="Session history">{data.sessions.map(session=><div key={session.session_id??session.member_run_id??session.team_run_id} className="border-b border-border/70 py-3"><div className="flex items-center justify-between gap-3"><b className="truncate text-[11px]">{shortId(session.session_id??session.member_run_id)}</b><Badge tone={session.runtime_status==="running"?"good":"muted"}>{session.runtime_status}</Badge></div><p className="mt-1 text-[9px] text-muted-foreground">{session.provider} · generation {session.runtime_generation??"host"} · {formatTime(session.last_active_at)}</p></div>)}</ProfileSection>
      <ProfileSection title="Workspace"><ContextFact label="Binding" value={c.workspace_binding?.id??"not bound"}/><ContextFact label="Path" value={c.workspace_binding?.locator??"not projected"}/></ProfileSection>
    </div>
  </section></div>;
}

function MobileSheet({title,onClose,children}:{title:string;onClose:()=>void;children:React.ReactNode}){return <div className="fixed inset-0 z-40 bg-black/25 lg:hidden" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section role="dialog" aria-modal="true" aria-label={title} className="absolute inset-y-0 right-0 w-[min(92vw,25rem)] overflow-y-auto border-l border-border bg-background shadow-2xl"><header className="sticky top-0 z-10 flex min-h-12 items-center justify-between border-b border-border bg-background px-4"><h2 className="text-sm font-semibold">{title}</h2><Button size="icon" variant="secondary" onClick={onClose}><X className="size-4"/></Button></header>{children}</section></div>}
...
function TagList({values,empty}:{values:string[];empty:string}){return values.length?<div className="flex flex-wrap gap-2">{values.map(value=><span key={value} className="rounded-md bg-secondary px-2 py-1 text-[10px]">{value}</span>)}</div>:<p className="text-[10px] text-muted-foreground">{empty}</p>}
function EmptyCanvas({title,detail}:{title:string;detail:string}){return <div className="grid min-h-64 place-items-center border-y border-dashed border-border/80 text-center"><div><Inbox className="mx-auto size-5 text-muted-foreground/65"/><h3 className="mt-3 text-[13px] font-semibold">{title}</h3><p className="mx-auto mt-1 max-w-md text-[11px] leading-5 text-muted-foreground">{detail}</p></div></div>}
function actionLabel(kind:string){return ({send_message:"Send message",assign_work:"Assign Work",rebind_work:"Reassign Work",close_member_run:"Close MemberRun",reopen_member_run:"Reopen MemberRun",retire_member_run:"Retire Agent from Team",resume_native_session:"Resume MemberRun on native session",reconcile_delivery:"Reconcile Work delivery",reconcile_message_delivery:"Reconcile Message delivery",request_gate_evaluation:"Request gate review",request_changes:"Request Work changes",accept_work:"Accept Work",cancel_work:"Cancel Work"} as Record<string,string>)[kind]??kind.replace(/_/g," ")}
