...
impl HarnessStore {
    fn require_detached_recovery_close_fence_unlocked(
        ...
        let exact = fence.authorizing_supervisor_id == supervisor_id
            ...
            // residency, activity and cycle-marker checks still prove no
            // live runtime owns this lane. Close must not be fenced on the
            // label alone; the shared
            // `AgentSession::is_at_terminal_cycle_boundary` is the one
    ...
    pub fn acquire_node_daemon_lease(
        ...
        if let Some(current) = current.as_ref() {
            if current.status != NodeDaemonLeaseStatus::Released {
                if current.daemon_id == daemon_id
                    && current.instance_id == instance_id
                    && current.status == NodeDaemonLeaseStatus::Active
                    && current.expires_unix_ms > now_unix_ms
                {
                    return Ok(current.clone());
                }
                let code = if current.daemon_id == daemon_id && current.instance_id == instance_id {
                ...
                return Err(StoreError::Conflict(format!(
                    ...
                    current.generation, current.daemon_id, current.instance_id, current.status
                )));
            }
        }
    ...
    /// supervisors. The exact same daemon instance may enter Draining after
    /// lease expiry because expiry removes effect authority but does not erase
    /// its obligation to settle the predecessor generation. A successor may
    /// acquire only after this exact generation publishes `Released`.
