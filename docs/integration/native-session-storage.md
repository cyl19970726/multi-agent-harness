...
| Kimi `kimi_acp` | real ACP session id captured | `~/.kimi-code/sessions/**/session_<id>/agents/main/wire.jsonl` | reviewed 0.31.0 prefers lightweight `session/resume`; `session/load` is a method-not-found compatibility fallback whose replay is drained before the next prompt | model/thinking selection, same-session resume, next-round mail, and cooperative `session/cancel` notification are current; live activity remains transient and native history is read on demand |
