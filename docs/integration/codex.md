...
Agent Team、Dynamic Workflow、Host 或 Company OS WorkItem。
...
Hook 输出必须进入 harness store 后才算 durable execution reference。Hook
观察到的事实可以帮助 reconciliation，但不能越过 Wave gate、executor-native
ownership、WorkItem lifecycle 或 Approval policy。
