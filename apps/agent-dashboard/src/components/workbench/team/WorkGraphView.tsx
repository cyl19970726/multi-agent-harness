...
function layoutGraph(graph:WorkGraph,visibleWorks:WorkSummary[],membersById:Map<string,MemberCapacitySummary>,selectedWorkId:string|undefined,shelfWidth:number){
  ...
  const positions=layoutComponents(visibleWorks.map((work)=>work.work_id),projectedEdges,shelfWidth);
...
function layoutComponents(ids:string[],edges:WorkGraph["edges"],shelfWidth:number){
  ...
  const positions=new Map<string,{x:number;y:number}>();let shelfX=0;let shelfY=0;let shelfHeight=0;const gap=42;
...
export function WorkGraphView({graph,visibleWorks,membersById,selectedWorkId,onSelectWork}:{graph:WorkGraph;visibleWorks:WorkSummary[];membersById:Map<string,MemberCapacitySummary>;selectedWorkId?:string;onSelectWork:(workId:string)=>void}){
  const containerRef=useRef<HTMLElement>(null);const [containerWidth,setContainerWidth]=useState(0);
  useLayoutEffect(()=>{const target=containerRef.current;if(!target)return;const update=()=>setContainerWidth(target.clientWidth);update();const observer=new ResizeObserver(update);observer.observe(target);return()=>observer.disconnect();},[]);
  const visibleIds=useMemo(()=>new Set(visibleWorks.map((work)=>work.work_id)),[visibleWorks]);
  const hasVisibleDependencies=useMemo(()=>graph.edges.some((edge)=>visibleIds.has(edge.prerequisite_work_id)&&visibleIds.has(edge.dependent_work_id)),[graph.edges,visibleIds]);
  const shelfWidth=Math.max(NODE_WIDTH,(containerWidth||1040)-32);
  const elements=useMemo(()=>layoutGraph(graph,visibleWorks,membersById,selectedWorkId,shelfWidth),[graph,visibleWorks,membersById,selectedWorkId,shelfWidth]);
  return <section ref={containerRef} aria-labelledby="work-graph-title" data-testid="team-work-graph" className="mt-3 min-w-0 max-w-full">
    <header className="mb-2 flex flex-wrap items-end justify-between gap-2"><div><h3 id="work-graph-title" className="company-editorial-title text-lg">Work graph</h3><p className="text-[11px] text-muted-foreground">{hasVisibleDependencies?"Pan, zoom and inspect hard dependencies.":"Inspect independent Works without inventing dependency links."} Layout is local; lifecycle and readiness remain server-authoritative.</p></div><p className="text-[10px] text-muted-foreground">{hasVisibleDependencies?"Tab reaches nodes and edges":"Tab reaches each Work"} · Enter selects</p></header>
    {!hasVisibleDependencies?<DisconnectedWorkGrid works={visibleWorks} membersById={membersById} selectedWorkId={selectedWorkId} onSelectWork={onSelectWork}/>:<div className="h-[clamp(28rem,58dvh,42rem)] min-w-0 max-w-full overflow-hidden rounded-xl border border-border bg-card" data-testid="work-graph-infinite-canvas">
      <ReactFlow<WorkFlowNode,Edge> nodes={elements.nodes} edges={elements.edges} nodeTypes={nodeTypes} fitView fitViewOptions={{padding:0.16,minZoom:0.5,maxZoom:1.1}} minZoom={0.35} maxZoom={2.2} panOnScroll zoomOnPinch zoomOnDoubleClick={false} nodesDraggable={false} nodesConnectable={false} elementsSelectable deleteKeyCode={null} onNodeClick={(_,node)=>onSelectWork(node.id)} proOptions={{hideAttribution:true}} aria-label="Work dependency graph infinite canvas" colorMode="system">
...
function DisconnectedWorkGrid({works,membersById,selectedWorkId,onSelectWork}:{works:WorkSummary[];membersById:Map<string,MemberCapacitySummary>;selectedWorkId?:string;onSelectWork:(workId:string)=>void}){return <div className="rounded-xl border border-border bg-card p-3 sm:p-4" data-testid="work-graph-disconnected"><div className="mb-3 flex items-start gap-2 rounded-lg border border-dashed border-border bg-secondary/20 px-3 py-2.5"><GitBranch className="mt-0.5 size-4 shrink-0 text-muted-foreground"/><div><p className="text-xs font-medium">No dependency links in this view</p><p className="mt-0.5 text-[11px] text-muted-foreground">These Works are independent. Add a hard prerequisite to place them on the dependency canvas.</p></div></div><div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,14.75rem),1fr))] gap-3">{works.map((work)=>{const ownerId=work.assignee_ref?.agent_member_id??work.owner_actor_ref?.id??"";return <button key={work.work_id} type="button" data-work-graph-node={work.work_id} aria-pressed={work.work_id===selectedWorkId} onClick={()=>onSelectWork(work.work_id)} className={cn("agent-team-panel min-h-[7.75rem] min-w-0 rounded-[10px] border bg-card p-3 text-left shadow-sm transition-[border-color,box-shadow] hover:border-primary/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",work.work_id===selectedWorkId&&"agent-team-selected ring-2 ring-primary/20")}><WorkCardContent work={work} owner={membersById.get(ownerId)}/></button>;})}</div></div>}

function WorkNode({data,selected}:NodeProps<WorkFlowNode>){const {work,owner,active}=data;return <div data-work-graph-node={work.work_id} className={cn("agent-team-panel h-full rounded-[10px] border bg-card p-3 text-left shadow-sm transition-[border-color,box-shadow]",(selected||active)&&"agent-team-selected ring-2 ring-primary/20")}>
  <Handle type="target" position={Position.Left} isConnectable={false} className="!size-2 !border-background !bg-border"/>
  <WorkCardContent work={work} owner={owner}/>
  <Handle type="source" position={Position.Right} isConnectable={false} className="!size-2 !border-background !bg-border"/>
</div>}

function WorkCardContent({work,owner}:{work:WorkSummary;owner?:MemberCapacitySummary}){const readiness=workReadinessPresentation(work);const Icon=readiness.icon;return <><div className="flex items-center gap-2"><span className="agent-team-phase-label shrink-0">{work.condition!=="normal"?work.condition:work.phase}</span><span className="ml-auto text-[9px] uppercase tracking-wider text-muted-foreground">{work.priority}</span></div><strong className="mt-2 block truncate text-[13px]" title={work.title||work.work_id}>{work.title||work.work_id}</strong><span className={cn("mt-1.5 flex items-center gap-1.5 text-[10px] font-medium",readiness.tone)}><Icon className="size-3"/>{readiness.label}</span><span className="mt-2 flex min-w-0 items-center gap-1.5 border-t border-border/70 pt-2 text-[9px] text-muted-foreground">{owner?<><Avatar name={owner.display_name} identity={owner.agent_member_ref.id} size="xs"/><span className="truncate">{owner.display_name}</span></>:<><Users className="size-3"/>Unassigned</>}<span className="ml-auto flex items-center gap-1"><GitBranch className="size-3"/>{work.prerequisite_work_ids.length} in · {work.successor_work_ids?.length??0} out</span></span></>}
