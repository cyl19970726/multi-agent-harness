...
export function TeamCapacityStrip({ summary, className }: { summary: TeamPressureSummary; className?: string }) {
  return (
    <section aria-label="Team capacity and pressure" data-testid="team-capacity-strip" className={cn("grid grid-cols-3 gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-5", className)}>
      {capacityFacts(summary).map((fact) => {
        const Icon = fact.icon;
        const pressured = (fact.id === "blocked" || fact.id === "needs-review") && fact.value > 0;
        return (
          <div key={fact.id} data-capacity-tile={fact.id} className="min-w-0 bg-card px-2.5 py-2.5 sm:px-3">
