...
- The thread id is currently recorded as `MemberRun.acp_session_id`; the target
  is a mode-aware `NativeSessionRef`. Resume capability must reflect the actual
  adapter path, and restart-time resume remains separately gateable.
