...
| Agent Team run | `team-run create/list/status/inbox/host-inbox/bind-host/ack/reconcile-delivery/add-member/rename-member/close-member/reopen-member/deactivate-member/start/send/resolve-interaction/events/complete/cancel` | Implemented | Runtime control plane for persistent MemberRuns and typed mail. Close freezes coordination and releases a managed runtime; Reopen resumes the same MemberRun/native session; Deactivate retires permanently. `--member-owned-path name:path` adds enforced write scope to reusable `--agent-team-id` members without dropping `agent_member_id`. |
