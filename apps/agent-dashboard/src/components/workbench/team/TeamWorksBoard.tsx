...
function WorkCard({work,ownerMember,selected,onSelect,register,prominent=false}:{work:WorkSummary;ownerMember?:MemberCapacitySummary;selected:boolean;onSelect:(id:string)=>void;register:(node:HTMLButtonElement|null)=>void;prominent?:boolean}) { return <button ref={register} type="button" {...(prominent ? {"data-priority-work":work.work_id} : {"data-work-card":work.work_id})} onClick={() => onSelect(work.work_id)} className={cn("agent-team-panel min-w-0 rounded-[10px] p-4 text-left hover:-translate-y-px hover:border-primary/25 hover:shadow-[0_10px_28px_rgb(83_57_38_/_0.065)]",selected && "agent-team-selected",prominent && "w-full bg-accent/35")}><div className="flex flex-wrap items-center justify-between gap-2"><PhaseMark work={work}/><span className="text-[9px] font-semibold uppercase tracking-[.09em] text-muted-foreground">{work.priority}</span></div><h4 className="company-editorial-title mt-3 break-words text-[17px] leading-[1.18]">{work.title || work.work_id}</h4><p className="mt-1.5 line-clamp-2 text-[12px] leading-[1.5] text-muted-foreground">{work.completion_criteria_markdown || "No projected completion criteria."}</p><div className="mt-4 grid grid-cols-2 gap-2 text-[10px] text-muted-foreground"><span className="flex items-center gap-1.5"><FileCheck2 className="size-3.5"/>{work.artifact_refs.length + work.check_refs.length} evidence</span><span className="flex items-center justify-end gap-1.5"><CheckCircle2 className="size-3.5"/>{work.gate_summary.passed}/{work.gate_summary.required} gates</span></div><div className="mt-3 flex min-w-0 items-center gap-2 border-t border-border/75 pt-3 text-[11px] text-muted-foreground">{ownerMember ? <><Avatar name={ownerMember.display_name} identity={`${ownerMember.agent_member_ref.id} ${ownerMember.role}`} size="sm" tone={ownerMember.runtime_state === "running" ? "running" : ownerMember.capacity === "available" ? "good" : "idle"}/><CircleDotLabel text={memberLabel(ownerMember)}/></> : <><Users className="size-3.5"/>Unassigned</>}<span className="ml-auto shrink-0">v{work.work_revision}</span></div><div className="mt-3 flex items-center gap-1.5 text-[11px] font-medium"><WorkStateMark work={work}/></div></button>; }
