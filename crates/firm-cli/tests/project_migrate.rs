...
fn migrate_preserves_records_and_payloads_and_marks_old_store() {
    ...
    let out = run_firm(&home, &repo, &["project", "migrate"]);
...
fn migrate_is_idempotent() {
    ...
    let first = run_firm(&home, &repo, &["project", "migrate"]);
    ...
    let second = run_firm(&home, &repo, &["project", "migrate"]);
...
fn migrate_with_no_local_store_fails_safely() {
    ...
    let out = run_firm(&home, &bare, &["project", "migrate"]);
    assert!(
        !out.status.success(),
...
fn migrate_does_not_flip_active_project_without_switch() {
    ...
    run_firm(&home, &other, &["project", "add", "--switch"]);
    ...
    let current = run_firm(&home, home.base(), &["project", "current"]);
    ...
    let out = run_firm(&home, &repo2, &["project", "migrate", "--switch"]);
