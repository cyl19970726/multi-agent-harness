...
| WorkItem and Assignment | `partial` | Native `WorkItem`, `Assignment`, append validation, HTTP resources, and Store/API tests | Native `Milestone`, `WorkItem.milestone_ref`, and `WorkItem.work_type` are absent even though the canonical product docs require them. |
...
| Approval, Governance and Agent interaction controls | `partial` | Approval Focus dispatches declared `approval.decide`, refreshes Store truth, supports approved/rejected branches, and keeps Request changes disabled. [`browser-action-transport.md`](browser-action-transport.md) and its four-state evidence cover denial, audits, and replay. | Replace the local operator capability with an actor-bound Human session; add native Request changes/follow-up Work semantics before enabling that control. Governance and Agent interaction remain missing. |
