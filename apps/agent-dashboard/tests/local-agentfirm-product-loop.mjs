...
for(const surface of ["GlobalWorkIndex","TeamWorkspace","HostConsole","AgentConversationWorkspace","OperatorView"])assert.ok(fs.existsSync(`apps/agent-dashboard/src/surfaces/${surface}.tsx`),`${surface} missing`);
for(const marker of ["<GlobalWorkIndex","<TeamWorkspace","<AgentConversationWorkspace","<OperatorView"])assert.ok(shell.includes(marker),`route missing ${marker}`);
for(const retired of ["MemberWorkbench","CompanyWorkIndex","Missions","TeamWarRoom","TeamWorks","MemberRuns","AgentTeamOrganization"])assert.ok(!new RegExp(`<${retired}[\\s/>]`).test(shell),`retired surface remains an active route: ${retired}`);
...
const combined=["GlobalWorkIndex","TeamWorkspace","HostConsole","AgentConversationWorkspace","OperatorView","RoleViewPrimitives"].map(name=>fs.readFileSync(`apps/agent-dashboard/src/surfaces/${name}.tsx`,"utf8")).join("\n");
