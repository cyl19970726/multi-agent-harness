...
`firm governance` is the project-portable home of documentation/skill
governance. The gate logic lives in the `firm-governance` crate (native Rust,
compiled into the `firm` binary), so any project the firm operates on — Go,
...
```text
firm governance check     [--root <path>] [--json]   # run the gates; exit 1 on a blocking failure
firm governance init      [--root <path>]             # write a starter .governance.toml
firm governance describe  [--root <path>]             # print the active config
```

`governance` is store-less: it never resolves or mutates a firm store, so it
runs identically inside a goal worktree, in CI, or in a non-firm repo. The
...
travels with the repo). It is NOT placed under `.firm/`, which is the
...
`.governance.toml` (scaffold one with `firm governance init`).

Shape (`schema = "agent_firm.governance.v1"`):

```toml
schema = "agent_firm.governance.v1"
...
schema = "agent_firm.docs_registry.v1"
...
`firm governance check` is byte-parity verified against
...
`firm-governance` crate carries a self-host test
...
- Full cut (done): the #168 doc-sync phase + the `registered_doc` gate now read
  the engine / config registry; CI's rust job runs `firm governance check`
