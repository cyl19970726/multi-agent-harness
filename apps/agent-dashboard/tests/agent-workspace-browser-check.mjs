...
const observation=(id,semanticKind,payload,at,position)=>({schema_version:"agentfirm.provider_native_event_record.v2",record_id:id,provider:"codex",adapter_version:"agentfirm.provider_event_adapter.v2",native_source_ref:`provider-source:fixture/${id}`,agent_member_id:"agent-mira",agent_session_id:"session-mira-current",agent_session_generation:3,node_daemon_id:"node-fixture",node_daemon_generation:4,provider_thread_id:"thread-mira",provider_turn_id:"turn-mira-1",provider_event_id:id,ordering_position:position,causal_parent_id:null,correlation_id:null,runtime_command_id:null,occurred_at:at,observed_at:at,native_event:{type:semanticKind,payload},source_content_fingerprint:`sha256:${createHash("sha256").update(id).digest("hex")}`,fragments:[{fragment_id:`${id}:fragment-0`,fragment_index:0,semantic_kind:semanticKind,lifecycle_phase:"terminal",completeness:"complete",effect_certainty:semanticKind.startsWith("tool_call_")?"applied":"none",visibility:"team_session",payload}]});
...
const memberProjection={schema_version:"agentfirm.provider_native_event_record.v2",agent_session_id:"session-mira-current",agent_session_generation:3,source_snapshot_fingerprint:`sha256:${createHash("sha256").update("fixture").digest("hex")}`,episodes:[{episode_id:"episode-mira-1",provider_turn_id:"turn-mira-1",records:memberObservations,terminal:true,incomplete:false}],truncated:false,availability:"available",unavailable_reason_code:null,disabled_reason:null,page:{limit:80,has_more:true,next_before_position:10}};
...
const olderMemberView=envelope("agent_workspace",{...memberView.data,session_event_projection:{...memberProjection,episodes:[{episode_id:"episode-mira-earlier",provider_turn_id:"turn-mira-earlier",records:[olderWithoutProviderTimestamp],terminal:true,incomplete:false}],page:{limit:80,has_more:false,next_before_position:null}}},actions);
const otherProjectObservation=observation("native-other-project","assistant_response",{type:"assistant_response",text:"Exact native event from the second Project only."},"2026-08-12T11:00:00Z",20);
const otherProjectMemberView=envelope("agent_workspace",{...memberView.data,session_event_projection:{...memberProjection,episodes:[{episode_id:"episode-other-project",provider_turn_id:"turn-other-project",records:[otherProjectObservation],terminal:true,incomplete:false}],page:{limit:80,has_more:false,next_before_position:null}}},actions);
...
try{
  ...
  const makePage=async token=>{const next=await browser.newPage({viewport:{width:1440,height:1000}});next.on("console",message=>{if(message.type()==="error")consoleErrors.push(message.text());});next.on("pageerror",error=>consoleErrors.push(error.message));next.on("response",response=>{if(response.status()>=400)httpFailures.push(`${response.status()} ${response.url()}`);});await next.addInitScript(({token,fixture,scope})=>{window.__AGENTFIRM_BOOTSTRAP__={capabilityToken:token};class QuietEventSource{timer;started=false;addEventListener(type,listener){if(!fixture||type!=="snapshot"||this.started)return;this.started=true;let revision=0;this.timer=setInterval(()=>{revision+=1;listener({data:JSON.stringify({generated_at:`2026-08-12T08:10:${String(revision).padStart(2,"0")}Z`,execution_space_id:"fixture-space"})});if(revision===80){clearInterval(this.timer);this.timer=undefined;}},15);}close(){if(this.timer)clearInterval(this.timer);}}Object.defineProperty(window,"EventSource",{value:QuietEventSource,configurable:true});if(fixture){const nativeFetch=window.fetch.bind(window);let memberStreamIssued=false;try{memberStreamIssued=sessionStorage.getItem("fixture-member-stream-issued")==="1";}catch{}window.fetch=async(input,init)=>{const request=input instanceof Request?input:null;const url=new URL(request?.url??String(input),location.href);if(url.pathname!=="/v1/events")return nativeFetch(input,init);const headers=new Headers(request?.headers);new Headers(init?.headers).forEach((value,key)=>headers.set(key,value));const capability=headers.get("X-AgentFirm-Token");const member=url.searchParams.get("agent_id")==="agent-mira"||capability==="fixture-member-token";const exactScope=member?scope.member:scope.host;const terminal={schema_version:"agentfirm.live_provider_activity_event.v2",reason:"terminal",scope:exactScope,activity:null};const encode=value=>new TextEncoder().encode(`event: live_provider_activity\ndata: ${JSON.stringify(value)}\n\n`);if(!member||memberStreamIssued)return new Response(encode(terminal),{headers:{"Content-Type":"text/event-stream"}});memberStreamIssued=true;try{sessionStorage.setItem("fixture-member-stream-issued","1");}catch{}let timer;const stream=new ReadableStream({start(controller){const now=Date.now();const record={schema_version:"agentfirm.provider_native_event_record.v2",record_id:"browser-live",provider:"codex",adapter_version:"agentfirm.provider_event_adapter.v2",native_source_ref:"provider-source:fixture/browser-live",agent_member_id:exactScope.agent_member_id,agent_session_id:exactScope.agent_session_id,agent_session_generation:exactScope.agent_session_generation,node_daemon_id:exactScope.node_daemon_id,node_daemon_generation:exactScope.node_daemon_generation,provider_thread_id:"thread-mira",provider_turn_id:"turn-mira-live",provider_event_id:"browser-live",ordering_position:1,causal_parent_id:null,correlation_id:null,runtime_command_id:null,occurred_at:null,observed_at:"2026-08-12T08:10:00Z",native_event:{type:"tool_execution_start",command:"cargo check"},source_content_fingerprint:`sha256:${"b".repeat(64)}`,fragments:[{fragment_id:"browser-live:fragment-0",fragment_index:0,semantic_kind:"tool_call_started",lifecycle_phase:"started",completeness:"partial",effect_certainty:"unknown",visibility:"team_session",payload:{type:"tool",tool_name:"cargo check",call_id:"browser-call",display_detail:"cargo check"}}]};const activity={schema_version:"agentfirm.live_provider_activity.v2",durability:"volatile_process_memory",replayable:false,...exactScope,runtime_snapshot_locator:"runtime-snapshot-1",expires_unix_ms:now+5000,items:[{runtime_event_locator:"runtime-event-1",record,emitted_unix_ms:now,expires_unix_ms:now+5000}]};controller.enqueue(encode({schema_version:"agentfirm.live_provider_activity_event.v2",reason:"updated",scope:exactScope,activity}));const staleScope={...exactScope,agent_session_id:"stale-session",agent_session_generation:exactScope.agent_session_generation+1};controller.enqueue(encode({schema_version:"agentfirm.live_provider_activity_event.v2",reason:"terminal",scope:staleScope,activity:null}));timer=setTimeout(()=>{controller.enqueue(encode(terminal));controller.close();},2500);},cancel(){if(timer)clearTimeout(timer);}});return new Response(stream,{headers:{"Content-Type":"text/event-stream"}});};}},{token,fixture:!liveConfig,scope:{member:{execution_space_id:"fixture-space",project_id:"fixture-project",team_run_id:team.latest_run.id,member_run_id:"member-run-mira",member_run_generation:3,agent_session_id:"session-mira-current",agent_session_generation:3,agent_member_id:"agent-mira",node_daemon_id:"node-fixture",node_daemon_generation:4},host:{execution_space_id:"fixture-space",project_id:"fixture-project",team_run_id:team.latest_run.id,member_run_id:"member-run-host",member_run_generation:1,agent_session_id:"host-thread-current",agent_session_generation:1,agent_member_id:"agent-host",node_daemon_id:"node-fixture",node_daemon_generation:4}}});if(!liveConfig)await next.route("**/v1/**",async route=>{
  ...
  if(!liveConfig){const liveSlot=page.locator(".aw-current-execution");await liveSlot.waitFor();await liveSlot.locator(".aw-native-event-body").filter({hasText:"cargo check"}).waitFor();await page.screenshot({path:join(evidenceDir,`member-live--1440x1000--${capturedSourceSha}.png`),animations:"disabled"});await page.waitForTimeout(150);assert.equal(await liveSlot.count(),1,"stale Session terminal cleared the current exact Session activity");await liveSlot.waitFor({state:"detached",timeout:4000});await page.getByText(/live stream disconnected/).waitFor();}
  ...
  const hostMemberLive=hostPage.locator(".aw-current-execution");await hostMemberLive.waitFor();await hostMemberLive.locator(".aw-native-event-body").filter({hasText:"cargo check"}).waitFor();
  ...
  const captures=[];
  for(const name of [...(!liveConfig?["member-live"]:[]),"member-session",...(!liveConfig?["member-event-detail"]:[]),"member-messages","member-work","member-configuration","host-session","host-member-team-session"]){
