...
export function AgentConversationWorkspace({
  ...
  return <Tooltip.Provider delayDuration={350}>
    <main ref={workspaceRef} className="agent-team-surface agent-workspace h-full min-h-0 flex-1 overflow-hidden" data-testid="agent-workspace">
      <div className="agent-workspace-layout grid h-full min-h-0 grid-cols-1 lg:grid-cols-[17.5rem_minmax(0,1fr)_18.125rem]">
...
function WorkCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(work:WorkSummary)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="agent-work-canvas mx-auto max-w-[60rem] px-5 pb-10 sm:px-7">
    ...
    {visible.length?<div className="agent-work-stream">{visible.map((work,index)=><button key={work.work_id} type="button" data-current={lens==="current"&&index===0||undefined} className="agent-work-row grid w-full grid-cols-[minmax(0,1fr)_7rem_7rem] gap-4 text-left" onClick={()=>onSelect(work)}><span className="min-w-0"><span className="flex items-center gap-2"><span className="truncate text-[13px] font-semibold">{work.title||work.work_id}</span>{work.condition!=="normal"&&<WorkspaceState label={humanizeToken(String(work.condition))} tone="bad"/>}</span><span className="aw-record-meta"><span>{shortId(work.work_id)} · v{work.work_revision}</span><span>{humanizeToken(String(work.priority))}</span></span><span className="mt-2 line-clamp-2 text-[12px] leading-5 text-foreground/75">{work.completion_criteria_markdown}</span></span><span><span className="agent-team-phase-label">{humanizeToken(work.phase)}</span>{work.delivery_summary.recovery_class&&work.delivery_summary.recovery_class!=="none"&&<span className="mt-2 block text-[10.5px] text-muted-foreground">{humanizeToken(String(work.delivery_summary.recovery_class))}</span>}</span><span className="text-right"><span className="text-[10.5px] font-semibold">{work.gate_summary.passed}/{work.gate_summary.required} gates</span><span className="mt-2 block text-[10.5px] text-muted-foreground">{formatTime(work.updated_at)}</span></span></button>)}</div>:<EmptyCanvas title="No Work in this view" detail="Eligibility is not ownership. Work remains authoritative in the Team Work kernel."/>}
  </div></ScrollArea.Viewport></ScrollArea.Root>;
}
...
function AgentComposer({data,actions,actionsCurrent,selectedRunId,onAction,onCompleted}:{data:AgentWorkspaceData;actions:AllowedAction[];actionsCurrent:boolean;selectedRunId:string|null;onAction:RoleActionExecutor;onCompleted:()=>void}){
  ...
  return <div data-testid="agent-workspace-composer" className="agent-workspace-composer shrink-0 border-t border-border bg-background/95">
...
function ProfileDialog({data,onClose,closeRef,openerRef}:{data:AgentWorkspaceData;onClose:()=>void;closeRef:React.RefObject<HTMLButtonElement>;openerRef:React.RefObject<HTMLButtonElement>}){
  ...
  return <div className="fixed inset-0 z-50 bg-[#3b2f27]/12" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section ref={dialogRef} role="dialog" aria-modal="true" aria-label={`${selected.display_name} configuration`} tabIndex={-1} className="agent-profile-drawer absolute inset-y-0 right-0 w-[min(92vw,29rem)] overflow-y-auto border-l border-border bg-background shadow-[0_0_28px_rgb(55_43_35_/_0.04)]">
    ...
    <div className="space-y-7 px-6 py-6"><ProfileSection title="Identity"><ContextFact label="AgentMember" value={selected.agent_member_ref.id}/><ContextFact label="Role" value={selected.role}/><ContextFact label="Organization" value={humanizeToken(selected.organization_status)} canonical={selected.organization_status}/>{c.description&&<p className="mt-3 text-[12px] leading-5 text-muted-foreground">{c.description}</p>}</ProfileSection>
      <ProfileSection title="Provider and permissions"><ContextFact label="Provider" value={selected.provider?humanizeToken(selected.provider):"Not bound"} canonical={selected.provider??undefined}/><ContextFact label="Execution mode" value={selected.execution_mode?humanizeToken(selected.execution_mode):"Not bound"} canonical={selected.execution_mode??undefined}/>{c.provider_profile_ref&&<ContextFact label="Provider profile" value={humanizeToken(c.provider_profile_ref)} canonical={c.provider_profile_ref}/>}<ContextFact label="Permission ceiling" value={c.permission_ceiling?humanizeToken(c.permission_ceiling):"Not projected"} canonical={c.permission_ceiling??undefined}/><ContextFact label="Workspace policy" value={c.workspace_policy?humanizeToken(c.workspace_policy):"Not projected"} canonical={c.workspace_policy??undefined}/></ProfileSection>
      <ProfileSection title="Skills, tools and capabilities"><ProfileList label="Skills" values={c.skill_refs} empty="No Skill refs are projected."/><ProfileList label="Configured tools" values={c.tool_refs} humanize empty={projectionSentence(c.tools_projection,"No configured tool allowlist is modeled on this AgentMember.")}/><ProfileList label="Capabilities" values={c.capabilities} humanize empty="No capabilities are projected."/></ProfileSection>
      <ProfileSection title="Prompt and restrictions">{c.prompt_ref?<ContextFact label="Prompt reference" value={c.prompt_ref}/>:<ContextFact label="Prompt" value={projectionLabel(c.prompt_projection)}/>}<ContextFact label="Forbidden actions" value={c.forbidden_actions.length?c.forbidden_actions.join(", "):projectionLabel(c.forbidden_actions_projection)}/><p className="mt-3 text-[10px] leading-5 text-muted-foreground">Unavailable fields remain explicit; the browser never infers configuration from provider behavior.</p></ProfileSection>
...
function TagList({values,empty,humanize=false}:{values:string[];empty:string;humanize?:boolean}){return values.length?<div className="flex flex-wrap gap-2">{values.map(value=><span key={value} title={value} className="rounded-md bg-secondary px-2 py-1 text-[11px]">{humanize?humanizeToken(value):value}</span>)}</div>:<p className="text-[11px] text-muted-foreground">{empty}</p>}
...
function projectionLabel(value:string){return value.startsWith("not_modeled")?"Not modeled":humanizeToken(value)}
function projectionSentence(value:string,fallback:string){return value.startsWith("not_modeled")?`${fallback} Canonical projection: ${value}.`:humanizeToken(value)}
