...
export function TeamMessageComposer({ actions, members, works, replyTo, teamId, teamRunId, actionsCurrent, onAction, onClearReply, onCompleted, collapsibleOnMobile=false, fixedRecipient, variant="panel", actionControl }: {
  ...
  if (variant === "conversation") return <section aria-labelledby="team-composer-title" className="bg-background/95 px-4 py-2"><div className="agent-team-composer mx-auto max-w-4xl"><button type="button" className="mb-1 flex min-h-7 w-full items-center justify-between text-[10px] font-semibold text-muted-foreground sm:hidden" aria-expanded={conversationDetailsOpen} onClick={() => setConversationDetailsOpen((open) => !open)}><span>To {fixedRecipient?.label ?? "Team"}</span><span>{conversationDetailsOpen ? "Hide details" : "Message details"}</span></button><div className={`${conversationDetailsOpen ? "grid" : "hidden sm:grid"} min-w-0 gap-2 border-b border-border/80 pb-2 sm:grid-cols-[10rem_minmax(7.5rem,9rem)_minmax(8rem,1fr)_auto] sm:items-center`}>{actionControl}<label className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-2 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground"><span>To</span>{fixedRecipient?<span className="truncate text-xs font-semibold normal-case text-foreground" title={fixedRecipient.label}>{fixedRecipient.label}</span>:<select value={recipientIds} onChange={(event)=>setRecipientIds(event.target.value)} title={members.find(member=>member.agent_member_ref.id===recipientIds)?.display_name} className="h-8 min-w-0 w-full rounded-md border border-border bg-card px-2 text-xs font-normal normal-case tracking-normal text-foreground"><option value="">Select Agent</option>{members.map(member=><option key={member.agent_member_ref.id} value={member.agent_member_ref.id}>{member.display_name}</option>)}</select>}</label><label className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-2 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground"><span className="shrink-0">Work</span><select value={workId} onChange={(event) => setWorkId(event.target.value)} className="h-8 min-w-0 w-full rounded-md border border-border bg-card px-2 text-xs font-normal normal-case tracking-normal"><option value="">No Work link</option>{works.map((work) => <option key={work.work_id} value={work.work_id}>{work.title || work.work_id}</option>)}</select></label><label className="flex min-h-8 items-center gap-1.5 whitespace-nowrap text-[9px] text-muted-foreground"><input type="checkbox" checked={responseRequired} onChange={(event) => setResponseRequired(event.target.checked)}/>Response</label></div>{replyTo && <p className="mt-1 truncate bg-primary/[0.04] px-2 py-1 text-[9px] text-muted-foreground">Correlated reply to {replyTo.sender.id} · {replyTo.message_id}</p>}<div className="flex min-w-0 items-end gap-2"><textarea aria-label="Message" value={body} onFocus={() => setConversationDetailsOpen(true)} onChange={(event) => setBody(event.target.value)} className="min-h-11 min-w-0 flex-1 resize-none border-0 bg-transparent px-1 py-3 text-sm outline-none placeholder:text-muted-foreground/70" rows={1} placeholder={`Write a message${fixedRecipient ? ` to ${fixedRecipient.label}` : ""}…`}/><Button size="sm" className="mb-1 min-h-9 shrink-0 px-3" disabled={!actionsCurrent || busy || !recipientIds || !body.trim() || Boolean(action?.disabled_reason)} title={!actionsCurrent ? "Awaiting a current authoritative view" : action?.disabled_reason ?? undefined} onClick={execute}><SendHorizontal className="size-3.5"/>{busy ? "Sending…" : replyTo ? "Reply" : "Send"}</Button></div><div className="flex items-center justify-between border-t border-border/75 pt-1.5"><p className="text-[9px] text-muted-foreground">Authenticated ordinary Message · never current-turn Steer</p><p className="hidden text-[9px] text-muted-foreground sm:block">Shift + Enter for a new line</p></div>{status && <p role="status" className="mt-1 text-[10px] text-muted-foreground">{status}</p>}</div></section>;
