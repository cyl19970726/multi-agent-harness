...
pub fn spaces_dir(harness_home: &Path) -> PathBuf {
    harness_home.join("execution-spaces")
}

pub fn space_store_root(harness_home: &Path, id: &str) -> PathBuf {
    spaces_dir(harness_home).join(id)
}

pub fn registry_path(harness_home: &Path) -> PathBuf {
    spaces_dir(harness_home).join("registry.json")
}

pub fn active_space_path(harness_home: &Path) -> PathBuf {
    harness_home.join("ACTIVE_SPACE")
}
...
impl ExecutionSpaceRegistry {
    pub fn load(harness_home: &Path) -> ExecutionSpaceResult<Self> {
        match std::fs::read_to_string(registry_path(harness_home)) {
    ...
    pub fn save(&mut self, harness_home: &Path) -> ExecutionSpaceResult<()> {
        self.format_version = REGISTRY_FORMAT_VERSION;
        std::fs::create_dir_all(spaces_dir(harness_home))?;
        std::fs::write(
...
pub fn context_for_id(
    ...
    let registry = ExecutionSpaceRegistry::load(harness_home)?;
    ...
    let store_root = space_store_root(harness_home, id);
...
pub fn register_and_activate(
    ...
    let context = ExecutionSpace {
        ...
        store_root: space_store_root(harness_home, id),
        default_project_binding_id,
        company_id,
    };
    write_metadata(&context)?;
    let mut registry = ExecutionSpaceRegistry::load(harness_home)?;
    registry.upsert(
    ...
    write_active_space(harness_home, &context.id)?;
    Ok(context)
}
...
pub fn switch_current_space(
    ...
    let context = context_for_id(harness_home, id)?.ok_or_else(|| {
...
pub fn active_space_id(harness_home: &Path) -> ExecutionSpaceResult<Option<String>> {
    let registry = ExecutionSpaceRegistry::load(harness_home)?;
    if registry.current_space_id.is_some() {
        return Ok(registry.current_space_id);
    }
    read_active_space(harness_home)
}

pub fn list_spaces(harness_home: &Path) -> ExecutionSpaceResult<Vec<ExecutionSpace>> {
    let registry = ExecutionSpaceRegistry::load(harness_home)?;
...
pub fn write_active_space(harness_home: &Path, id: &str) -> ExecutionSpaceResult<()> {
    std::fs::create_dir_all(harness_home)?;
    std::fs::write(active_space_path(harness_home), format!("{id}\n"))?;
    Ok(())
}

pub fn read_active_space(harness_home: &Path) -> ExecutionSpaceResult<Option<String>> {
    match std::fs::read_to_string(active_space_path(harness_home)) {
        Ok(text) => {
...
pub fn harness_home() -> ExecutionSpaceResult<PathBuf> {
    project::harness_home().map_err(|error| match error {
