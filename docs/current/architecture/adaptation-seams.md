...
types and packages in the current implementation; the links below name their source seams.
...
| Layer | Owns | Packages |
| --- | --- | --- |
| Outer — Harness coordination (durable, provider-neutral) | identity, Work, Message, RuntimeCommand facts and policy | `firm-core` (domain types, immutable ids), `firm-store` (canonical facts, CAS, lease, append/rebuild), `firm-application` (Role Actions and Views, typed outcomes, application ports), `firm-node-daemon` (machine authority, lease renewal, socket and drain lifecycle), with `DaemonApplicationPort` implemented by `firm-cli` (`daemon_application`, `team_supervision`, `runtime_composition`) alongside CLI/HTTP/Dashboard adapters, `firm-fabric` (cross-machine transport) |
...
| --- | --- | --- | --- |
| S1 Capability declaration and admission | `ProviderIntegrationProfile` snapshot per MemberRun; `review_required` is a refusal, not a warning | nine `SemanticCapability` values, each bound to provider + execution mode + provider version + adapter revision with a status and evidence string; `CapabilityResolver`, `AdmissionDecision`, `RuntimeBindingFence` | exact provider version, protocol, primitive availability ("provider supports" ≠ "adapter supports") |
...
| S4 `ExecutionCycle` | Supervisor wake/claim → drive → settle; durable `ProviderCycleCorrelation` including its closed `ending` | `TeamRuntimeAdapter::run_cycle`: input-acceptance receipt, control injection, terminal boundary, `ExecutionCycleOutcome`; `CycleTimeouts` separates input acceptance from control settlement (liveness is proven structurally, not bounded by a clock); typed `CycleSettlement` preserves receipt and interruption evidence; `CycleEnding` is the closed table of how a cycle ends and keys both `member_actions.action_type` and `provider_status`, with each adapter mapping its own `Err` endings through one wildcard-free match and draining them via `take_cycle_ending` (DEV-156, ADR 0076; `firm-runtime-contract/src/{cycle,cycle_ending,timeouts}.rs`) | one native turn / prompt / query / followup and its terminal frame |
...
| S9 Conformance | one shared admission and lifecycle discipline every adapter must pass | `conformance.rs`: the fenced `RuntimeAdapter` trait, `preflight_effect` (fence plus capability-closure admission), `CompositionLifecycle` (quiesce verified before a composition swap), `OneShotDisposer`. The cycle-level rules live where stated: receipt before terminal and transport loss after receipt → Unknown in [agent-runtime.md](agent-runtime.md); provider completion or empty output is not semantic success in [agent-integration-model.md](agent-integration-model.md) and [agent-team-dogfood-loop.md](../product/agent-team-dogfood-loop.md); Close only on exact owned evidence as the typed `MemberRuntimeCloseReceipt::verify()` in `receipt_and_terminal.rs` (all five close postconditions Satisfied, else `MemberCloseIncomplete`) and `conformance.rs`'s fail-closed `close_runtime` default; the stale-idle scenario only in SPEC-ARCH-BOUNDARY-01's conformance gate (Notion), not yet in a repository document or assertion. The five-provider native-control seam test in `firm-cli` exercises the first two; shared provider-parameterised assertions are implemented in `firm-runtime-contract/src/cycle_assertions.rs` and exercised by the provider packages; the CLI seam test is `five_provider_native_control_seam_is_durable_replay_safe_and_fail_closed.rs` | each provider's scripted fixtures |
...
additionally declares one binding outside the shared nine
(`observe_native_queue`, Supported) — a read-only `get_state` snapshot, all
that survives ADR 0068's retirement of Pi's `steer` and `follow_up`.
