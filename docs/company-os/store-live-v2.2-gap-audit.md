...
| Approval, Governance and Agent interaction controls | `missing` | The UI honestly renders disabled controls and explains that no approved transport is connected | Add an authenticated, governed command seam, refresh the authoritative snapshot after accepted effects, and expose denial/error/audit results. Start with `approval.decide`; do not enable Payment as part of the pending trademark scenario. |
