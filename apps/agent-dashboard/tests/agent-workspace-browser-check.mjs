...
const observation=(id,semanticKind,payload,at,position)=>({schema_version:"agentfirm.provider_observation.v1",observation_id:id,provider:"codex",adapter_version:"agentfirm.provider_event_adapter.v1",native_source_ref:`provider-source:fixture/${id}`,agent_identity_id:"agent-mira",agent_session_id:"session-mira-current",agent_session_generation:3,node_daemon_id:"node-fixture",node_daemon_generation:4,provider_thread_id:"thread-mira",provider_turn_id:"turn-mira-1",provider_event_id:id,ordering_position:position,causal_parent_id:null,correlation_id:null,runtime_command_id:null,occurred_at:at,observed_at:at,semantic_kind:semanticKind,lifecycle_phase:"terminal",completeness:"complete",effect_certainty:semanticKind.startsWith("tool_call_")?"applied":"none",visibility:"team_session",redacted:false,truncated:false,native_event:{type:semanticKind,payload},source_content_fingerprint:`sha256:${id.padEnd(64,"0").slice(0,64)}`,payload});
const memberObservations=[
  observation("native-0","authored_response",{type:"authored_response",text:"I mapped the Agent Workspace read model to the approved composition and started with the privacy boundary."},"2026-08-12T07:58:00Z",10),
...
const olderMemberView=envelope("agent_workspace",{...memberView.data,session_event_projection:{...memberProjection,episodes:[{episode_id:"episode-mira-earlier",provider_turn_id:"turn-mira-earlier",observations:[observation("native-earlier","authored_response",{type:"authored_response",text:"Earlier exact provider-native event loaded from the same native Session."},"2026-08-12T07:50:00Z",1)],terminal:true,incomplete:false}],page:{limit:80,has_more:false,next_before_position:null}}},actions);
...
try{
  ...
  if(!liveConfig){
    ...
    await eventRow.locator('xpath=ancestor::div[@data-boundary-aligned="true"]').locator(".aw-native-event-body").filter({hasText:"Validated Team-scoped native Session"}).waitFor();
  ...
  const hostMemberTool=hostMemberEvents.nth(1);await hostMemberTool.click();await hostMemberTool.locator('xpath=ancestor::div[@data-boundary-aligned="true"]').locator(".aw-native-event-body").filter({hasText:"Validated Team-scoped native Session"}).waitFor();
