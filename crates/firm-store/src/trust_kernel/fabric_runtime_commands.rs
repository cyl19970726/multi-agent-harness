...
impl HarnessStore {
    ...
    pub fn runtime_command_is_publicly_recoverable(
        ...
        if record.phase != RuntimeCommandPhase::Prepared
            || record.effect_certainty != RuntimeEffectCertainty::Unknown
        {
            return Ok(false);
        }
    ...
    pub fn resolve_runtime_command_recovery(
        ...
        let is_recovery_required = record.phase == RuntimeCommandPhase::RecoveryRequired
            && record.effect_certainty == RuntimeEffectCertainty::Unknown;
        let is_abandoned_prepared = record.phase == RuntimeCommandPhase::Prepared
            && record.effect_certainty == RuntimeEffectCertainty::Unknown
