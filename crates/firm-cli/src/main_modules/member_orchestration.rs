...
pub(crate) fn ensure_team_runtime_fabric(
        ...
        .filter(|member| !member.is_external_interactive())
    {
        ...
        if let Some(session) = current_sessions.first() {
            ...
            let expected_native_session = expected_member_run_native_session_ref(member);
            if !session_native_session_is_admissible(
            ...
            let native_session_ref = expected_member_run_native_session_ref(member);
