...
monotonic WorkExecutionBinding and WorkDelivery. A request-changes review — by
the Host, or by the exact peer of Host-owned Work — returns Review → Open; the
scheduler then admits the next exact binding/delivery generation. A compatible
Workspace and provider-native session may continue, but continuity never
substitutes for exact runtime admission.
