...
This document defines the canonical workflow runtime for `crates/firm-workflow`: a provider-neutral Starlark execution layer that lets the firm run authored multi-agent programs, journal their leaves, and expose the result as `WorkflowRun` / `WorkflowStep` records.
...
`AgentStepSpec` is the provider-neutral leaf description produced by
`agent()`, `parallel()`, and `pipeline()`. It carries:
...
| `--initiated-by <id>` | Initiator id; defaults to `FIRM_AGENT_MEMBER_ID` or `operator` |
...
A `run-script` invocation journals into the firm store in this order:
...
Ephemeral provider workers run in their own process group so the firm can tear down the whole worker tree, not just the immediate CLI process, when an idle or per-leaf wall timeout fires. A worker that exits normally is cleaned up in-process, including removal of its temporary worker pidfile.
