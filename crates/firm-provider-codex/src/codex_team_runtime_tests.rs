...
// cohesive boundary rather than an arbitrary cut. Declared here, not in
// team_runtime.rs, so the split is invisible to the production module.
#[path = "codex_cycle_ending_tests.rs"]
mod cycle_ending;
