...
| Agent control plane | The provider-neutral contract in [agent-control-plane.md](agent-control-plane.md) is reflected in CLI/API behavior: durable identity, create/start/close lifecycle, busy/idle queues, peer messages, reducer output, and Dashboard control actions. | A member roster with no queue semantics, no peer-message ledger, or provider events that cannot be reduced into harness objects. |
