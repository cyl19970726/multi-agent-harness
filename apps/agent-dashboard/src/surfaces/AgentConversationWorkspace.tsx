          ...
          <header className="agent-workspace-header flex min-h-[5.65rem] shrink-0 items-center gap-3 border-b border-border px-4 sm:px-6">
            ...
            <button ref={profileTriggerRef} type="button" className="group flex min-w-0 flex-1 items-center gap-3 text-left" onClick={()=>setProfileOpen(true)} aria-label={`Open ${selected.display_name} configuration`}>
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 flex min-w-0 items-center gap-2 text-[11px] text-muted-foreground"><span className="truncate">{selected.role}</span><span>·</span>{publicProjection?<span>Public coordination view</span>:<><span>{selected.provider ?? "native provider unavailable"}</span><span>·</span><span className="truncate">{data.selected_session_id ?? "Native Session unavailable"}</span></>}</span>
              </span>
            </button>
...
function MessagesCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(next:ContextSelection)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="mx-auto max-w-[58rem] px-5 pb-10 pt-5 sm:px-8">
    <div className="agent-message-toolbar mb-4 flex flex-wrap items-center gap-2 border-b border-border pb-4"><div className="flex rounded-lg bg-secondary/70 p-1">{(["all","inbox","outbox","unread"] as const).map(item=><button key={item} type="button" data-active={lens===item} onClick={()=>setLens(item)} className="rounded-md px-3 py-1.5 text-[10px] font-semibold capitalize text-muted-foreground data-[active=true]:bg-background data-[active=true]:text-foreground data-[active=true]:shadow-sm">{item}</button>)}</div><label className="relative ml-auto min-w-[14rem] flex-1 sm:max-w-[20rem]"><Search className="pointer-events-none absolute left-3 top-2.5 size-3.5 text-muted-foreground"/><span className="sr-only">Search messages</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search messages" className="h-9 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-xs"/></label></div>
    {visible.length?<div className="divide-y divide-border/75">{visible.map(message=>{const actor=data.roster.find(item=>item.agent_member_ref.id===message.sender.id);const actorName=actor?.display_name??message.sender.id;const sparse=visible.length<=2;return <button key={message.message_id} type="button" className="agent-message-row grid w-full grid-cols-[2rem_minmax(0,1fr)_8rem] gap-3 py-4 text-left" onClick={()=>onSelect({kind:"message",message})}><Avatar name={actorName} identity={`${message.sender.id} ${actor?.role??""}`} size="sm" tone={actor?.runtime_state==="running"?"running":"idle"}/><span className="min-w-0"><span className="flex items-center gap-2"><b className="truncate text-[11px]">{actorName}</b><span className="text-[9px] text-muted-foreground">{message.sender.id===selectedId?"Outbox":message.kind}</span>{message.work_id&&<Badge tone="muted">{shortId(message.work_id)}</Badge>}</span><span className={`mt-1 text-[12px] leading-5 text-foreground/85 ${sparse?"block max-w-[42rem] whitespace-pre-wrap":"line-clamp-3"}`}>{message.body}</span></span><span className="text-right text-[9px] text-muted-foreground"><span className="block">{formatTime(message.created_at)}</span><span className="mt-2 block">{message.deliveries.map(delivery=>delivery.status).join(", ")||"recorded"}</span></span></button>})}</div>:<EmptyCanvas title="No matching messages" detail="Inbox and Outbox stay in Harness coordination records; provider-private transcripts are not mirrored here."/>}
...
function ProfileDialog({data,onClose,closeRef,openerRef}:{data:AgentWorkspaceData;onClose:()=>void;closeRef:React.RefObject<HTMLButtonElement>;openerRef:React.RefObject<HTMLButtonElement>}){
  ...
  return <div className="fixed inset-0 z-50 bg-[#3b2f27]/12" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section ref={dialogRef} role="dialog" aria-modal="true" aria-label={`${selected.display_name} configuration`} tabIndex={-1} className="agent-profile-drawer absolute inset-y-0 right-0 w-[min(92vw,29rem)] overflow-y-auto border-l border-border bg-background shadow-[0_0_28px_rgb(55_43_35_/_0.04)]">
    ...
    <div className="space-y-7 px-6 py-6"><ProfileSection title="Identity"><ContextFact label="AgentMember" value={selected.agent_member_ref.id}/><ContextFact label="Role" value={selected.role}/><ContextFact label="Organization" value={selected.organization_status}/>{c.description&&<p className="mt-3 text-[12px] leading-5 text-muted-foreground">{c.description}</p>}</ProfileSection>
      ...
      <ProfileSection title="Skills, tools and capabilities"><ProfileList label="Skills" values={c.skill_refs} empty="No Skill refs are projected."/><ProfileList label="Configured tools" values={c.tool_refs} empty={`No configured tool allowlist is modeled on AgentMember · ${c.tools_projection}.`}/><ProfileList label="Capabilities" values={c.capabilities} empty="No capabilities are projected."/></ProfileSection>
...
function ProfileList({label,values,empty}:{label:string;values:string[];empty:string}){return <div className="mt-3 first:mt-0"><p className="mb-1.5 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">{label}</p><TagList values={values} empty={empty}/></div>}
function EmptyCanvas({title,detail,compact=false}:{title:string;detail:string;compact?:boolean}){return <div className={`grid place-items-center border-y border-dashed border-border/80 text-center ${compact?"min-h-[4.5rem]":"min-h-64"}`}><div className={compact?"flex items-start gap-3 px-4 text-left":""}><Inbox className={`${compact?"mt-0.5 size-4":"mx-auto size-5"} shrink-0 text-muted-foreground/65`}/><div><h3 className={`${compact?"":"mt-3"} text-[12px] font-semibold`}>{title}</h3><p className="mx-auto mt-1 max-w-md text-[10px] leading-4 text-muted-foreground">{detail}</p></div></div></div>}
