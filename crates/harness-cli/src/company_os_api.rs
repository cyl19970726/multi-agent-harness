    ...
    assignments.into_iter().map(|assignment| {
        ...
        let member = if correlation_matches {
            exact_message.and_then(|message| message.get("to_member_ids")).and_then(Value::as_array)
                .and_then(|ids| ids.iter().filter_map(Value::as_str).find_map(|id| members.iter().find(|m| value_text(m, "id") == id && m.get("agent_member_id").and_then(Value::as_str) == durable_agent_member)))
        } else { None };
        let link_status = if evidence_ref.is_empty() || exact_message.is_none() || agent_member_ref.is_none() || durable_member_count == 0 { "unavailable" } else if claimant_count != 1 || durable_member_count != 1 || !correlation_matches || member.is_none() { "mismatch" } else { "linked" };
        ...
        let handoffs = member.map(|m| messages.iter().filter(|v| value_text(v, "kind") == "handoff" && value_text(v, "correlation_id") == correlation && value_text(v, "from_member_id") == value_text(m, "id")).map(|v| {
...
mod projection_tests {
    ...
    fn work_execution_chain_requires_exact_links_and_reports_freshness() {
        ...
        let messages = vec![
            ...
            json!({"id": "handoff-1", "kind": "handoff", "correlation_id": "corr-exact", "from_member_id": "member-exact", "body": "RESULT: completed\nDelivery evidence attached.", "created_at": "2026-07-31T00:00:00Z", "evidence_refs": ["check-1"]}),
        ];
        let records = vec![
            json!({"id": "pr-fresh", "record_type": "github_pull_request_ref", "title": "PR", "fields": {"work_item_id": "work-1", "assignment_id": "assignment-exact", "repository": "owner/repo", "pull_request_number": "42", "head_ref": "codex/fix", "head_sha": "abc123", "base_ref": "master", "url": "https://example.test/pr/42", "state": "open", "observed_at": "2026-07-31T00:00:00Z", "source_updated_at": "2026-07-30T23:00:00Z", "source_completed_at": "", "observed_unix_ms": "9900", "freshness_ttl_ms": "200"}}),
        ...
        let orphan = build_work_assignment_execution_chains(
            vec![assignment(
                "assignment-exact",
                "corr-exact",
                "message-exact",
            ...
            vec![
                json!({"id": "message-exact", "kind": "assignment", "correlation_id": "corr-exact", "to_member_ids": ["member-exact"]}),
            ],
            vec![],
            10_000,
        );
        ...
        let duplicate_claim = build_work_assignment_execution_chains(
            vec![assignment(
                "assignment-exact",
                "corr-exact",
                "message-exact",
        ...
        let duplicate_member = build_work_assignment_execution_chains(
            vec![assignment(
                "assignment-exact",
                "corr-exact",
                "message-exact",
...
/// Executor or closer standing, using the same actor set as
/// `validate_work_item_transition`. Contributors are deliberately excluded:
...
fn controls_work_item(actor: &ActorRef, item: &WorkItem) -> bool {
    *actor == item.accountable_owner
        || item.assignees.contains(actor)
        || item.reviewer.as_ref() == Some(actor)
}

/// The executor/closer roles the requesting Actor would gain for itself.
fn self_granted_roles(
    actor: &ActorRef,
    previous: &WorkItem,
    target: &WorkItem,
) -> Vec<&'static str> {
...
/// update authority may perform governed routing. Neither path may be used to
/// hand the requesting Actor executor or closer standing it did not already
/// hold, so `work_item.update` can never be used to pass the
/// `work_item.transition` ownership check.
fn require_work_item_responsibility_authority(
    ...
    let controlled_before = controls_work_item(requester, previous);
    let laundered = self_granted_roles(requester, previous, target);

    // Checked before the authority paths so that self-elevation is refused
    // even for an Actor that holds policy-named routing authority. Routing
    // work to someone else stays separable from taking the work.
    if !controlled_before && !laundered.is_empty() {
        return Err(deny_work_item_update(
            ...
            format!(
                "Actor {} cannot use work_item.update to grant itself {} on WorkItem {}: \
                 the requesting Actor is not already its accountable_owner, an assignee, \
                 or its reviewer, and work_item.update must never create the ownership \
                 that work_item.transition checks",
                requester.actor_id,
                laundered.join(", "),
                previous.id
            ),
...
fn record_action_denial(
    ...
    match store.claim_action_command(&denied)? {
        ActionCommandClaimResult::Claimed(_) => {}
        ActionCommandClaimResult::Replay(existing) => {
            if existing.status != ActionCommandStatus::Requested {
                return Ok(());
    ...
    store.append_action_command(&denied)?;
