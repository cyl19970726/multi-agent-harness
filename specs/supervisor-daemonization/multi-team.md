...
BEFORE (per-team foreground supervisor):
  $ firm team-run start --id run-A    # process blocks, drives A
  $ firm team-run start --id run-B    # separate process, drives B

AFTER (multi-team daemon):
  $ firm daemon serve                 # one long-lived process
  $ firm team-run start --id run-A    # sends control message to daemon
  $ firm team-run start --id run-B    # sends control message to daemon
```
...
`firm team-run start --id <run-id>` changes behavior:
...
`firm daemon serve` is the new command that runs the multi-team
daemon.  The existing `firm daemon start|status|stop` commands
(which manage the resident claude pool) are renamed to `firm daemon
...
This design builds on top of the per-run detached supervisor daemon
already delivered in #365 (`crates/firm-cli/src/supervisor_daemon.rs`,
567 lines).  The per-run daemon currently manages exactly one team-run
per process; this design extends it to manage N team-runs in a single
process.

### Modified Files

- `crates/firm-cli/src/supervisor_daemon.rs` — extend from per-run to
  multi-team: add scan loop, context registry, control socket server,
  crash-recovery adoption logic
- `crates/firm-cli/src/main.rs`:
  - Route `firm daemon serve` to the multi-team daemon entry point
  - Change `firm team-run start` to prefer daemon when socket present

### Tests

- `crates/firm-cli/tests/team_run_daemon.rs` — integration tests
  covering:
  ...
  - Graceful shutdown (SIGTERM drains active runs, all threads join
    with deadline)
