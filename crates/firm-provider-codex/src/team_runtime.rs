...
pub struct CodexTeamRuntime<'a, B = CodexAppServerClient> {
    bridge: B,
...
impl<'a, B: CodexAppServerBridge> CodexTeamRuntime<'a, B> {
    ...
    pub fn with_provider_request_handler(
        mut self,
                    ...
                    match method {
                        ...
                        Some("turn/completed") => {
                            ...
                            if terminal.status == "interrupted" && interrupt.is_none() {
                                return Err(CliError::Usage(format!(
                                    "CODEX_RUNTIME_POSTCONDITION_UNKNOWN: turn {turn_id} ended as interrupted without a Harness control request"
                                )));
                            }
                            self.active_turn_id = None;
                            self.exact_thread_is_idle(false)?;
...
impl<'a, B: CodexAppServerBridge> harness_runtime_contract::RuntimeAdapter
