...
historical work design / outcome evaluation / goal-case learning layer. The value is the
...
[0003 minimal-first-types](../../../decisions/0003-minimal-first-types.md) and
[0015 evidence/message/decision-first](../../../decisions/0015-autonomous-proposals-use-evidence-message-decision.md)
...
- `Goal`: `vision_id`, `historical work design_id`, `closed_by_decision_id`.
...
- **legacy plan record** — the executable goal thesis (scenario, non-goals, risk and
  permission boundaries, required infra, legacy dependency graph, evidence plan, acceptance
  gates).
- **outcome evaluation** — the retrospective (outcome, what worked / failed,
...
> first-class `legacy phase record` object owned by `Goal.phases[]`, and a task joins its
...
> `crates/harness-core/src/lib.rs` (`struct legacy phase record`, `Task.phase_id`). The
...
- Harness core gains generic shapes (Review, Gap, legacy plan record, outcome evaluation,
