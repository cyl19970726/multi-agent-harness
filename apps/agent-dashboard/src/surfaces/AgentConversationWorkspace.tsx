...
function MessagesCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(next:ContextSelection)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="mx-auto max-w-[58rem] px-5 pb-10 pt-5 sm:px-8">
    ...
    {visible.length?<div className="divide-y divide-border/75">{visible.map(message=>{const actor=data.roster.find(item=>item.agent_member_ref.id===message.sender.id);const actorName=actor?.display_name??message.sender.id;return <button key={message.message_id} type="button" className="agent-message-row grid w-full grid-cols-[2rem_minmax(0,1fr)_8rem] gap-3 py-4 text-left" onClick={()=>onSelect({kind:"message",message})}><Avatar name={actorName} identity={`${message.sender.id} ${actor?.role??""}`} size="sm" tone={actor?.runtime_state==="running"?"running":"idle"}/><span className="min-w-0"><span className="flex items-center gap-2"><b className="truncate text-[11px]">{actorName}</b><span className="text-[9px] text-muted-foreground">{message.sender.id===selectedId?"Outbox":message.kind}</span>{message.work_id&&<Badge tone="muted">{shortId(message.work_id)}</Badge>}</span><span className="mt-1 line-clamp-2 text-[12px] leading-5 text-foreground/85">{message.body}</span></span><span className="text-right text-[9px] text-muted-foreground"><span className="block">{formatTime(message.created_at)}</span><span className="mt-2 block">{message.deliveries.map(delivery=>delivery.status).join(", ")||"recorded"}</span></span></button>})}</div>:<EmptyCanvas title="No matching messages" detail="Inbox and Outbox stay in Harness coordination records; provider-private transcripts are not mirrored here."/>}
