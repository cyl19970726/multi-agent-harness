...
try {
  ...
  if(binary){
    ...
    const overlap=overlapGenerationsFixture();
    overlap.evidence.revision=structuredClone(f.evidence.revision);
    overlap.records.find(r=>r.operation.event.aggregate_kind==='work_report').operation.resulting_projection.candidate.value=f.evidence.revision.candidate;
    const saveTupleInput=()=>{writeFileSync(evidence,JSON.stringify(overlap.evidence));writeFileSync(join(selectedRoot,'agentfirm_trust_operations.jsonl'),jsonl(overlap.records));writeFileSync(join(selectedRoot,'team_runs.jsonl'),jsonl(overlap.sources.teamRuns));};
    saveTupleInput();
    const tuplePass=verify(join(selectedRoot,'agentfirm_trust_operations.jsonl'));assert.equal(tuplePass.status,0,tuplePass.stderr);
    overlap.evidence.sessions.reverse();saveTupleInput();assert.equal(verify(join(selectedRoot,'agentfirm_trust_operations.jsonl')).status,0,'tuple claims need not be ordered by generation');
    const actual=overlap.evidence.sessions.find(s=>s.agent_member_id==='member-fixture');
    overlap.evidence.sessions.unshift({...actual,session_generation:2,tool_started:99,tool_terminal:99});actual.tool_terminal=0;saveTupleInput();
    const borrowed=verify(join(selectedRoot,'agentfirm_trust_operations.jsonl'));assert.notEqual(borrowed.status,0);assert.match(borrowed.stderr,/exact implementer binding requires a terminal tool result/);
    actual.tool_terminal=1;overlap.evidence.sessions.shift();
    overlap.evidence.sessions.push({...actual,native_session_id:'conflicting'});saveTupleInput();
    const duplicate=verify(join(selectedRoot,'agentfirm_trust_operations.jsonl'));assert.notEqual(duplicate.status,0);assert.match(duplicate.stderr,/unique exact Session ID\/generation tuples/);
    writeFileSync(evidence,JSON.stringify(f.evidence));writeFileSync(join(selectedRoot,'agentfirm_trust_operations.jsonl'),jsonl(f.records));writeFileSync(join(selectedRoot,'team_runs.jsonl'),jsonl(f.sources.teamRuns));
    console.log('v2 real CLI tuple regression PASS: Review84/gen1 and Accept88/gen2, reversed rows, exact implementer counters, duplicate conflict refusal');
