...
import { formatDate, formatAbsolute, isoTime, shortId } from "@/components/workbench/team/teamFormat";
import type { MemberCapacitySummary, MessageSummary, TeamActivitySummary } from "../../../model/roleViews";

export type ActivityFilters = {query:string;source:string;participant:string;workId:string;responseOnly:boolean};
export const emptyActivityFilters:ActivityFilters={query:"",source:"all",participant:"all",workId:"all",responseOnly:false};

export function TeamConversationStream({ activity, messages, members=[], truncated, onOpenWork, onReply, filters, onFiltersChange }: {
  ...
  const [localFilters,setLocalFilters]=useState(emptyActivityFilters);
  const {query,source,participant,workId,responseOnly}=filters??localFilters;
  const updateFilters=(patch:Partial<ActivityFilters>)=>{const next={...(filters??localFilters),...patch};setLocalFilters(next);onFiltersChange?.(next);};
  const setQuery=(query:string)=>updateFilters({query});
  const setSource=(source:string)=>updateFilters({source});
  const setParticipant=(participant:string)=>updateFilters({participant});
  const setWorkId=(workId:string)=>updateFilters({workId});
...
function MessageRow({message,membersById,onOpenWork,onReply}:{message:MessageSummary;membersById:Map<string,MemberCapacitySummary>;onOpenWork:(id:string)=>void;onReply?:(message:MessageSummary)=>void}) {
  ...
  const longBody=message.body.length>600;
  const statuses = Array.from(new Set(message.deliveries.map((delivery) => delivery.status)));
  const stateLabel = message.delivery_state ? message.delivery_state.replace(/_/g," ") : statuses.join(" · ");
  const stateTone = message.delivery_state === "acknowledged" || message.delivery_state === "delivered" ? "good" : message.delivery_state === "failed" ? "bad" : message.delivery_state ? "info" : statuses.includes("provider_received") ? "good" : statuses.length ? "info" : "muted";
  const sender = membersById.get(message.sender.id);
  const senderLabel = message.sender.display_name ?? sender?.display_name ?? message.sender.id;
  const recipientLabels = message.recipients.map((recipient) => recipient.display_name ?? membersById.get(recipient.id)?.display_name ?? recipient.id);
  return <AgentTeamAuthoredTurn className="grid min-w-0 gap-3 px-3 py-3 lg:grid-cols-[7rem_minmax(9rem,.72fr)_minmax(15rem,2fr)_minmax(7rem,.7fr)_7rem] lg:items-start"><RecordType icon={MessageSquare} label="Message" detail={message.kind.replace(/_/g," ")} tone="coral"/><Actor member={sender} fallback={senderLabel} detail={<><ArrowRight className="size-3"/>{recipientLabels.join(", ") || "Team"}</>}/><div className="min-w-0"><div className="text-sm leading-relaxed">{longBody&&!expanded?<p className="whitespace-pre-wrap">{message.body.slice(0,320)}…</p>:<Markdown source={message.body} compact/>}</div>{longBody&&<button type="button" className="mt-2 text-xs font-medium text-primary" aria-expanded={expanded} onClick={()=>setExpanded(value=>!value)}>{expanded?"Collapse message":"Read full message"}</button>}<div className="mt-1.5 flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground"><span className="font-mono">chain {shortId(message.correlation_id)}</span>{message.causation_id && <span>reply to {shortId(message.causation_id)}</span>}<details><summary className="cursor-pointer text-primary">{stateLabel || (message.deliveries.length ? "" : "no delivery rows")}</summary>{message.deliveries.length > 0 && <ul className="mt-1 space-y-1 bg-secondary p-2">{message.deliveries.map((delivery) => <li key={delivery.id} className="break-all">recipient {delivery.recipient_display_name ?? delivery.recipient_identity_id ?? delivery.recipient_member_run_id} · {delivery.status} · v{delivery.version}{delivery.provider_receipt_id ? ` · receipt ${delivery.provider_receipt_id}` : ""}</li>)}</ul>}</details>{onReply && message.reply_eligible && <button type="button" className="font-semibold text-primary hover:underline" onClick={() => onReply(message)}>Reply</button>}</div></div><WorkLink workId={message.work_id} onOpenWork={onOpenWork}/><RowState createdAt={message.created_at} label={stateLabel || "authored"} tone={stateTone}/></AgentTeamAuthoredTurn>;
}
...
function WorkLink({workId,onOpenWork}:{workId:string|null;onOpenWork:(id:string)=>void}) { return <div>{workId ? <button type="button" data-activity-work-id={workId} className="max-w-full truncate text-[11px] text-muted-foreground hover:text-primary" onClick={() => onOpenWork(workId)}>Work · {workId}</button> : <span className="text-[10px] text-muted-foreground">No Work link</span>}</div>; }
function RowState({createdAt,label,tone}:{createdAt:string;label:string;tone:"good"|"info"|"muted"|"bad"|"warn"}) { return <div className="text-left lg:text-right"><time className="block text-[11px] text-muted-foreground" dateTime={isoTime(createdAt)} title={formatAbsolute(createdAt)}>{formatDate(createdAt)}</time><Badge className="mt-1" tone={tone}>{label}</Badge></div>; }
