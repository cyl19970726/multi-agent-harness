...
/// harness store itself stays isolated via `HARNESS_HOME`.
fn host_harness_json(home: &TempHome, project_id: &str, args: &[&str]) -> serde_json::Value {
    let mut full = vec!["--project", project_id];
    full.extend_from_slice(args);
    let out = run_harness_with_env(home, home.base(), &full, &[("HOME", &real_home())]);
    assert!(
        out.status.success(),
        "harness {args:?} failed: {}",
...
fn member_harness_json(
    ...
    let out = run_harness_with_env(
        ...
        &[
            ("HOME", &real_home()),
            ("HARNESS_TEAM_RUN_ID", run_id),
            ("HARNESS_MEMBER_RUN_ID", member_id),
        ],
    );
    assert!(
        out.status.success(),
...
fn github_issue_and_pr_linkage_roundtrip() {
    ...
    let submitted = member_harness_json(
        &home,
        &project_id,
        &run_id,
        &member_id,
