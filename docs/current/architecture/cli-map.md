...
| Agent Team run | `team-run create/list/status/work/inbox/host-inbox/bind-host/host-lease-status/renew-host-lease/release-host-lease/ack/reconcile-delivery/add-member/rename-member/close-member/reopen-member/deactivate-member/start/send/resolve-interaction/events/complete/cancel` | Implemented | Runtime control plane for shared Works, persistent MemberRuns, WorkDelivery, and typed conversation. `bind-host` durably records the exact Host surface/thread independently from `HostBindingLease`; only trusted exact runtime discovery (currently matching Codex `CODEX_THREAD_ID`) creates an Interactive lease, while unsupported or unverifiable Claude/Kimi/manual identity remains bound but unleased with an actionable warning. Lease status/renew/release expose TTL and exact owner/lease/generation fencing. A live Interactive lease suppresses dispatcher scheduling; the foreground scheduler may materialize stale-binding attention and report `DispatchReady`, but never claims or claims to wake a Host. Actual claims require an exact Dispatcher lease plus an explicit consumer; the persistent/headless provider driver remains #415/later, so full #387 and terminal accept/merge/cancel are outside this triage slice. `team-run work` exposes list/show/create/assign/claim/start/block/submit/request-changes/accept/cancel. Close freezes coordination and releases a managed runtime; Reopen resumes the same MemberRun/native session; Deactivate retires permanently. |
