...
fn pi_rpc_handshake_and_basic_prompt() {
    ...
    let state_response = read_response(&mut child, "t1", Duration::from_secs(10))
        .expect("get_state response");

    let data = state_response
        .get("data")
        .expect("get_state data field");
    ...
    let auto_compaction = data
        .get("autoCompactionEnabled")
        .and_then(|v| v.as_bool())
        .expect("autoCompactionEnabled");
    assert!(auto_compaction, "autoCompactionEnabled should be true by default");
    ...
    let output_path = tmp.path().join("pi-canary.txt");
    let prompt = format!(
    ...
    for line in BufReader::new(stdout).lines() {
        ...
        let frame: serde_json::Value =
            serde_json::from_str(&line).expect("parse JSON frame");
