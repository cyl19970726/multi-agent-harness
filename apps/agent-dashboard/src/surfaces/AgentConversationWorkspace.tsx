...
import {
  Activity, ArrowLeft, BriefcaseBusiness, ChevronRight, Circle,
          ...
          <Tabs.Root value={mode} onValueChange={value=>{setContextSelection(null);onSelectionChange({agentWorkspaceMode:value as WorkspaceMode});}} className="flex min-h-0 flex-1 flex-col">
            <div data-testid="agent-workspace-modebar" className="aw-modebar flex min-h-12 shrink-0 items-end border-b border-border px-4 sm:px-7">
              <Tabs.List aria-label="Agent Workspace modes" className="agent-workspace-tabs flex h-full items-end gap-7">
                <WorkspaceTab value="session" icon={Sparkles} label="Session" count={sessionProjection?.episodes.reduce((count,episode)=>count+episode.observations.length,0)??0}/>
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-transparent">
    ...
    <ScrollArea.Root className="min-h-0 flex-1 overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="px-2 pb-5">
      {visible.map((agent,index)=><div key={agent.agent_member_ref.id}>{index===0&&<p className="px-3 pb-1 pt-2 text-[9px] font-semibold uppercase tracking-[.15em] text-muted-foreground">Host Agent</p>}{index===1&&<p className="px-3 pb-1 pt-5 text-[9px] font-semibold uppercase tracking-[.15em] text-muted-foreground">Team Members</p>}<button type="button" onClick={()=>onSelect(agent)} data-selected={agent.agent_member_ref.id===selectedId} className="agent-roster-row group flex w-full items-center gap-3 px-3 py-3 text-left">
        <Avatar name={agent.display_name} identity={`${agent.agent_member_ref.id} ${agent.role}`} size="lg" tone={agent.runtime_state==="running"?"running":agent.capacity==="available"?"good":"idle"}/>
        <span className="min-w-0 flex-1"><span className="flex items-start gap-2"><span className="agent-roster-name min-w-0 break-words text-[13.5px] font-semibold leading-4">{agent.display_name}</span>{agent.is_host&&<span className="shrink-0 text-[9px] font-semibold text-primary">Host</span>}</span><span className="agent-roster-meta mt-1 block break-words text-[10.5px] leading-4 text-muted-foreground">{humanizeToken(agent.role)} · {humanizeToken(agent.runtime_state??agent.capacity??"unknown")}</span></span>
...
function SessionCanvas({data,liveActivity,onSelect}:{data:AgentWorkspaceData;liveActivity:LiveProviderActivity|null;onSelect:(next:ContextSelection)=>void}){
...
function NativeFactsTrail({events,selectedEventId,onOpen}:{events:ProviderObservation[];selectedEventId:string|null;onOpen:(event:ProviderObservation)=>void}){
  const containsSelected=events.some(event=>event.observation_id===selectedEventId);
  return <details className="aw-native-facts-trail" open={containsSelected||undefined}>
    <summary className="flex items-center gap-2"><TerminalSquare aria-hidden="true"/><span className="aw-native-facts-trail__count">{events.length} native {events.length===1?"observation":"observations"}</span><span className="aw-native-facts-trail__hint">tool · runtime · display-safe reasoning</span><time className="ml-auto">{formatTime(observationTime(events[0]))}{events.length>1&&observationTime(events[events.length-1])!==observationTime(events[0])?` – ${formatTime(observationTime(events[events.length-1]))}`:""}</time></summary>
    <div className="aw-native-facts-trail__body">{events.map(event=><ExpandableEvent key={event.observation_id} event={event} selected={selectedEventId===event.observation_id} onSelect={()=>onOpen(event)}/>)}</div>
  </details>;
}
...
function NativeAuthoredRecord({event,actorName,selected,onSelect}:{event:ProviderObservation;actorName:string;selected:boolean;onSelect:()=>void}){
  ...
  return <article role="button" tabIndex={0} aria-label={`Open native Session response from ${actorName}`} data-selected={selected||undefined} className="aw-native-authored-record" onClick={onSelect} onKeyDown={eventKey=>activateOnKeyDown(eventKey,onSelect)} onKeyUp={eventKey=>activateOnKeyUp(eventKey,onSelect)}>
    ...
    <div className="min-w-0 flex-1"><header><strong>{actorName}</strong><span>Provider-native response</span><time>{formatTime(observationTime(event))}</time></header><p>{copy.summary}</p><footer>{humanizeToken(event.provider)} · {humanizeToken(event.completeness)} · native source retained by provider</footer></div>
  </article>;
}
  ...
  return <div className="agent-message-stream">{threads.map(thread=>{
    ...
    return <section key={thread.correlationId} className="aw-message-thread" aria-label={`Conversation about ${linkedWork?.title??"unlinked coordination"}`}>
      ...
      <div className="aw-message-thread__turns">{thread.items.map((message,index)=>{
        ...
        const delivery=message.deliveries.map(item=>humanizeToken(item.status)).join(", ");
        return <button key={message.message_id} type="button" data-thread-continuation={index>0||undefined} className="agent-message-row flex w-full gap-3 text-left" onClick={()=>onSelect({kind:"message",message})}><Avatar name={actorName} identity={`${message.sender.id} ${actor?.role??""}`} size="md" tone={actor?.runtime_state==="running"?"running":"idle"}/><span className="min-w-0 flex-1"><span className="flex items-baseline gap-2"><b className="truncate text-[12.5px]">{actorName}</b><span className="aw-record-kind">{message.sender.id===selectedId?"Outbox":"Inbox"} → {recipients}</span><time className="ml-auto text-[10.5px] text-muted-foreground">{formatTime(message.created_at)}</time></span><span className="mt-1.5 block max-w-[42rem] whitespace-pre-wrap text-[13.5px] leading-[1.58] text-foreground/90">{message.body}</span><span className="aw-record-meta">{message.causation_id&&<span>Reply</span>}{delivery&&<span>Delivery · {delivery}</span>}</span></span></button>;
      })}</div>
    </section>;
  })}</div>;
}
...
function AgentContextRail({view,data,mode,selected,currentWork,actions}:{view:RoleView<AgentWorkspaceData>;data:AgentWorkspaceData;mode:WorkspaceMode;selected:ContextSelection;currentWork?:WorkSummary;actions:AllowedAction[]}){
  ...
  const responsibilitySection=!isHost&&<ContextSection title="Responsibility" hint={eligibleWorks.length?`${eligibleWorks.length} eligible Work`:undefined}><ResponsibilityStrip values={responsibility}/>{latestExchange&&<div className="mt-3"><ContextMessageRow data={data} message={latestExchange}/></div>}</ContextSection>;
  ...
  const sessionSection=selfPrivate?<ContextSection title="Current Session" hint={privateProjection?.agent_session_id?humanizeToken(data.selected_agent.runtime_status??"available"):"Unavailable"}><ContextFact label="Provider" value={data.selected_agent.provider?humanizeToken(data.selected_agent.provider):"Not bound"}/><ContextFact label="Session" value={shortId(privateProjection?.agent_session_id)}/><ContextFact label="Episodes" value={String(privateProjection?.episodes.length??0)}/><ContextFact label="Last activity" value={formatTime(data.context_summary.last_activity_at)}/></ContextSection>:<ContextSection title="Privacy"><div className="aw-privacy-notice"><ShieldCheck aria-hidden="true"/><p>This view includes public Messages and Work only. The selected Agent's private Session, tools and runtime are structurally absent.</p></div></ContextSection>;
  ...
  const sections=mode==="messages"
    ?[workSection,selectionInset,conversationSection,evidenceSection,controlsSection,sessionSection]
    :mode==="work"
      ?[workSection,selectionInset,responsibilitySection,evidenceSection,needsHostSection,assignedSection,controlsSection,sessionSection]
      :[workSection,selectionInset,responsibilitySection,needsHostSection,inboxSection,assignedSection,evidenceSection,sessionSection,controlsSection];
...
function ProfileDialog({data,onClose,closeRef,openerRef}:{data:AgentWorkspaceData;onClose:()=>void;closeRef:React.RefObject<HTMLButtonElement>;openerRef:React.RefObject<HTMLButtonElement>}){
  ...
  return <div className="fixed inset-0 z-50 bg-[#3b2f27]/12" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section ref={dialogRef} role="dialog" aria-modal="true" aria-label={`${selected.display_name} configuration`} tabIndex={-1} className="agent-profile-drawer absolute inset-y-0 right-0 w-[min(92vw,29rem)] overflow-y-auto border-l border-border bg-background shadow-[0_0_28px_rgb(55_43_35_/_0.04)]">
    ...
    <div className="space-y-7 px-6 py-6"><ProfileSection title="Who"><ContextFact label="Role" value={humanizeToken(selected.role)} canonical={selected.role}/><ContextFact label="Organization" value={humanizeToken(selected.organization_status)} canonical={selected.organization_status}/>{c.description&&<p className="aw-profile-description">{c.description}</p>}<p className="aw-profile-canonical" title={selected.agent_member_ref.id}>Durable AgentMember · {shortId(selected.agent_member_ref.id)}</p></ProfileSection>
      ...
      <ProfileSection title="Runtime">{hasProviderConfiguration?<><ContextFact label="Provider" value={selected.provider?humanizeToken(selected.provider):"Not bound"} canonical={selected.provider??undefined}/><ContextFact label="Execution mode" value={selected.execution_mode?humanizeToken(selected.execution_mode):"Not bound"} canonical={selected.execution_mode??undefined}/>{c.provider_profile_ref&&<ContextFact label="Provider profile" value={humanizeToken(c.provider_profile_ref)} canonical={c.provider_profile_ref}/>}</>:<p className="aw-profile-empty">No provider runtime is projected for this Agent.</p>}{sessionProjection&&<div className="aw-profile-current-session"><div><span>Current Session projection</span><Badge tone={sessionProjection.agent_session_id?"good":"muted"}>{sessionProjection.agent_session_id?"Available":"Unavailable"}</Badge></div><strong>{selected.provider?humanizeToken(selected.provider):"Provider not bound"}</strong><p>{sessionProjection.episodes.length} native episodes · <span title={sessionProjection.agent_session_id??undefined}>{shortId(sessionProjection.agent_session_id)}</span></p></div>}{c.workspace_binding&&<div className="mt-3"><ContextFact label="Workspace" value={c.workspace_binding.status?humanizeToken(c.workspace_binding.status):"Bound"} canonical={c.workspace_binding.id}/>{c.workspace_binding.locator&&<ContextFact label="Path" value={c.workspace_binding.locator}/>}</div>}</ProfileSection>
