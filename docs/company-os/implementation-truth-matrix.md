...
| Organization | `organization-and-actors.md`; Human, Standing Agent, External, Service, OrgUnit and Membership; optional `company-org-operator` skill | `schemas/company-os/actors.schema.json` | typed actor and organization ledgers with reference validation | resource reads plus flat `harness company org list/query/create-human/create-agent/create-unit/add-membership/transition-actor/update-permissions` and nested `harness company org actor/unit/membership ...` administrative authoring; no governed Org/HR lifecycle proposal/approval Action family yet | Store-live organization plus a shared FocusShell Standing Agent workspace show identity, reporting projection, configuration refs, assigned work and durable activity | core/store/API reference tests, Dashboard type/build checks, navigation checks, `check-company-os-org-cli-smoke`, and operator CLI smoke | **partial; Organization CLI v1 and Standing Agent profile verified** — native prompt/tool/skill/Docs/WorkType/escalation refs, OrgUnit membership, declared status, permission/capability ref maintenance, and distinct execution boundary exist; governed organization evolution proposal/approval does not |
...
- Baseline Organization CLI authoring is Actual, but governed Organization
  ...
  shared Standing Agent workspace is Actual; promotion, retirement and
  permission-change workflows must remain planned until their Action and
  acceptance chains exist.
