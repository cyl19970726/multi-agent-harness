...
| Organization | `organization-and-actors.md`; Human, Standing Agent, External, Service, OrgUnit and Membership; optional `company-org-operator` skill | `schemas/company-os/actors.schema.json` | typed actor and organization ledgers with reference validation | `harness company org list/query/create-human/create-agent/create-unit/add-membership/transition-actor/update-permissions`; writes use the current Human administrative authoring boundary; no governed OrgChangeProposal lifecycle Action family yet | Store-live organization plus a shared FocusShell Standing Agent workspace show identity, reporting projection, configuration refs, assigned work and durable activity | core/store/API reference tests, Dashboard type/build checks, navigation checks, and `check-company-os-org-cli-smoke` | **partial; Organization CLI v1 and Standing Agent profile verified** — native prompt/tool/skill/Docs/WorkType/escalation refs, OrgUnit membership, declared status, and permission/capability ref maintenance exist; governed organization evolution proposal/approval does not |
...
| Finance | `financial-relations.md`; Commitment and Payment stay separate; optional `company-finance-operator` skill | `schemas/company-os/finance.schema.json` | separate Commitment and Payment ledgers with monotonic validation | `harness company finance list/query/propose-commitment/request-approval/decide-approval/transition-commitment/record-payment/transition-payment`; `propose-commitment` creates the initial proposed Commitment through the existing Human administrative import boundary in v1; approval requests, approval decisions, Commitment transitions, and Payment records/transitions use governed Actions | Finance and Approval views show Store-live monetary state and explicitly distinguish commitment from payment | core/store/API financial boundary tests, browser approval checks, and `check-company-os-finance-cli-smoke` | **partial overall; verified for native Commitment, approval, and prepared Payment CLI flow** — proposed Commitment creation is native Store truth but still administrative in v1; no Payment is inferred from an approved Commitment, and Payment requires separate approval/evidence |
