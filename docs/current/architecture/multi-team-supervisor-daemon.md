...
(`crates/firm-cli/src/main_modules/daemon_predecessor_recovery.rs:76`). Before
...
(`crates/firm-node-daemon/src/supervisor_daemon/machine_authority.rs:157-250`).
...
latching (`…/machine_authority.rs:201-213`); the caller then logs
...
"all-or-nothing" as "every lease or nothing", never as "any error at all".
...
continues
(`crates/firm-node-daemon/src/supervisor_daemon/team_supervision.rs:41-49`).
...
| Provider input-acceptance boundary | 300 s (`--idle-timeout-secs`) | `crates/firm-cli/src/main_modules/daemon_cli.rs:275-284` |
| Host binding lease TTL | 30 s default, 5–300 s accepted, renewed only by an explicit CLI call | `crates/firm-cli/src/main_modules/host_binding.rs:3-5`, `crates/firm-cli/src/main_modules/team_run_cli.rs:700-706` |
