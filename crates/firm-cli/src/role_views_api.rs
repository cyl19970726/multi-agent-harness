...
impl Facts {
    ...
    fn read_for_teams(
        ...
        Ok(Self {
            ...
            store_identity,
            sequence,
            work_sequence: work_records.len() as u64,
...
/// Order one Team's activity feed, newest first.
///
/// Rows come from four sources that stamp time in two formats: `unix-ms:` on
/// CLI coordination records and RFC3339 on imported/API ones. As raw strings
/// those never interleave — every ISO value sorts below every `unix-ms:` value
/// — so the whole feed is ordered through one parsed key. An unparseable stamp
/// sorts last rather than having an order invented for it, and the tie-break
/// stays deterministic: newer Work revision first, then id.
fn sort_activity_rows(rows: &mut [Value]) {
    let key = |row: &Value| {
        row["created_at"]
            .as_str()
            .and_then(harness_core::work_acceptance::recorded_time_key)
    };
    rows.sort_by(|left, right| {
        key(right)
            .cmp(&key(left))
            .then_with(|| {
                right["resulting_version"]
                    .as_u64()
                    .cmp(&left["resulting_version"].as_u64())
            })
...
fn team_activity(
    ...
    for event in facts.work_display_events.iter().filter(|event| {
        ...
        rows.push(json!({"source":"work_event","id":event["id"],"work_id":event["work_id"],"actor_ref":role_actor_ref(facts,&event["performed_by_actor"]),"status":event["kind"],"summary":display_text(event["payload"].get("summary").and_then(Value::as_str)),"created_at":event["created_at"],"resulting_version":event["resulting_version"]}));
    }
