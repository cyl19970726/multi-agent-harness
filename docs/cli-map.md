...
| Assignment delivery | `work assign`; `work assignment status --assignment <id> --execution-space <space>`; `work assignment reconcile --assignment <id> --execution-space <space> --team-message <id> --delivery-state <state>` | Implemented | Appends Assignment routing and monotonic delivery revisions. Agent Team execution joins only through exact StandingAgent→AgentMember→MemberRun identity, exact TeamMessage id/correlation, and a delivered/acknowledged target-recipient receipt; CLI/UI surface delivery drift and terminal-execution-versus-pending health. It never rewrites `WorkItem.assignees` or auto-completes Work. |
