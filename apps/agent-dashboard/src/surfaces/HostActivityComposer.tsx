...
export function HostActivityComposer({apiUrl,space,project,routeIdentity,teamRunId,replyTo,refreshKey,actionsCurrent,onAction,onClearReply}:{
  apiUrl:string;
  space:string;
  project:string;
  ...
  return <TeamMessageComposer actions={view.allowed_actions} members={view.data.member_capacity} works={view.data.all_works} replyTo={replyTo} teamId={view.data.team_ref} teamRunId={resolvedRunId} actionsCurrent={actionsCurrent && view.freshness === "current"} onAction={onAction} onClearReply={onClearReply} onCompleted={() => setRefresh((value) => value+1)}/>;
}
