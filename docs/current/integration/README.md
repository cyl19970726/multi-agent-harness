...
| Kimi (Moonshot) | [kimi.md](kimi.md) · [ACP Team runtime](kimi-agent-team.md) | adapter implemented; installed 0.39.0 current for reviewed slices | `kimi_acp` is the Team mode. DEV-125 re-proved prompt delivery, K3/max controls, exact same-session resume with attach replay drained, bounded full-access receipts, cooperative `session/cancel`, and narrow `session/close` plus clean process reap. |
