...
| R3 (done) | Old `document/block/template` CLI command tree and `document.append`/`block.append` API actions deleted (not deprecated-forwarded); template Documents remain readable legacy records; the record-layer commands (`module`, `typed-record`, `view`, `relation`, health, source sync) remain — they are the shared record layer, not old docs | check-company-os-docs-cli-live retired and check-company-os-docs-cli-smoke rewritten to assert the v2 surface; v2 suites green; trademark scenario explicitly grandfathered: its seed writes through the generic raw `/v1/company-os/documents|blocks` resource endpoints (retained) and rendering flows through the v2 legacy projection |
