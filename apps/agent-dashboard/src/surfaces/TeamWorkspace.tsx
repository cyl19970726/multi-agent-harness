...
export function TeamWorkspace({apiUrl,space,project,teamId,teamRunId,refreshKey,selection,onAction,actionsCurrent,onSelectionChange}:{apiUrl:string;space:string;project:string;teamId:string;teamRunId?:string;refreshKey?:string;selection:SelectionState;onAction:RoleActionExecutor;actionsCurrent:boolean;onSelectionChange:(next:Partial<SelectionState>)=>void}){
  ...
  return <main className="agent-team-surface h-full flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-5" data-testid="authenticated-team-workspace"><div className="mx-auto max-w-[1500px]">
    <header className="border-b border-border px-1 pb-3"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="agent-team-eyebrow flex flex-wrap items-center gap-2"><Users className="size-3.5"/>Agent Team <span className="text-muted-foreground">· {team.viewer_role} view</span></div><h1 className="company-editorial-title mt-1 break-words text-2xl tracking-[-0.025em] sm:text-[30px]">{team.display_name || team.team_id}</h1><p className="mt-1 text-[12px] text-muted-foreground">Mission {team.mission_id} <span className="px-1.5">·</span> latest attempt {team.latest_run?.id ?? "not started"}</p></div><div className="flex flex-wrap items-center justify-end gap-2"><Button aria-label="Host conversation" size="sm" variant="outline" onClick={() => onSelectionChange({teamMode:"workspace",teamConversation:"host",memberRunId:undefined,teamTab:"members"})}><MessageSquare className="size-3.5"/><span className="sm:hidden">Host chat</span><span className="hidden sm:inline">Host conversation</span></Button>{team.viewer_role === "host" && <Button aria-label="Open Host Console" size="sm" variant="outline" aria-controls="host-tools" onClick={() => openHostTools()}><ShieldCheck className="size-3.5"/><span className="sm:hidden">Host Console</span><span className="hidden sm:inline">Open Host Console</span></Button>}<details className="relative"><summary className="flex min-h-8 cursor-pointer list-none items-center rounded-md border border-border bg-card px-3 text-xs font-medium">Context</summary><div className="agent-team-panel absolute right-0 top-10 z-30 w-72 rounded-xl p-4 shadow-xl"><dl className="space-y-2 text-xs"><ContextFact label="Team" value={team.team_id}/><ContextFact label="Mission" value={team.mission_id}/><ContextFact label="Host Agent" value={team.host_agent_id}/><ContextFact label="Node" value={team.node_id}/><ContextFact label="Run" value={team.latest_run?.id ?? "not started"}/><ContextFact label="Project" value={team.latest_run?.project_binding_id ?? "not attached"}/></dl><p className="mt-3 border-t border-border pt-3 text-[10px] leading-relaxed text-muted-foreground">Mission Log owns Host judgment. Provider transcripts remain native.</p></div></details><span className="hidden sm:inline-flex"><ViewProvenance view={view}/></span></div></div>
      <div className="mt-4 hidden border-t border-border pt-3 sm:grid sm:grid-cols-[minmax(0,1.12fr)_minmax(30rem,1fr)] sm:gap-8"><dl className="grid grid-cols-5 gap-x-5 gap-y-3 text-[10px] text-muted-foreground"><HeaderFact label="Run status" value={team.latest_run?.status ?? "not started"}/><HeaderFact label="Attempt" value={team.latest_run?.id ?? "not started"}/><HeaderFact label="Placement" value={`${team.node_id}${team.placement_generation != null ? ` · g${team.placement_generation}` : ""}`}/><HeaderFact label="Project binding" value={team.latest_run?.project_binding_id ?? "not attached"}/><HeaderFact label="Workspace" value={team.latest_run?.execution_root ?? "not attached"}/></dl><TeamCapacityStrip summary={view.data.pressure_summary} compact/></div>
    </header>
    ...
    <TeamCapacityStrip summary={view.data.pressure_summary} compact className="border-b border-border py-3 sm:hidden"/>
    <div className="min-w-0">
    <AgentTeamTabs value={tab} onValueChange={(teamTab) => onSelectionChange({teamTab})} label="Team Workspace sections">{TABS.map(({id,label,icon:Icon}) => <AgentTeamTab key={id} id={`team-workspace-tab-${id}`} value={id} aria-controls={`team-workspace-panel-${id}`}><Icon className="size-3.5"/>{label}{id === "activity" && view.data.messages.length > 0 && <span className="rounded-full bg-primary/10 px-1.5 text-[9px] text-primary">{view.data.messages.length}</span>}</AgentTeamTab>)}</AgentTeamTabs>
