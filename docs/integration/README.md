...
| Kimi (Moonshot) | [kimi.md](kimi.md) · [ACP Team runtime](kimi-agent-team.md) | adapter implemented; installed 0.31.0 current for reviewed slices | `kimi_acp` is the Team mode. Prompt delivery, K3 model/thinking controls, same-session resume, next-round batched mail, and the ACP `session/cancel` notification are reviewed. |
