...
pub const AGENT_SESSION_DRAIN_RESUME_NOT_YET_RESUMABLE: &str =
    "AgentSession interrupted by a NodeDaemon drain may resume only from a detached, disarmed lane at a terminal cycle boundary with no ambiguous RuntimeCommand; reconcile the runtime first";
...
pub const AGENT_SESSION_RECOVERY_REQUIRED_NOT_YET_RESUMABLE: &str =
    "AgentSession in RecoveryRequired may resume only from a detached, disarmed lane at a terminal cycle boundary with no ambiguous RuntimeCommand; reconcile the runtime first";
