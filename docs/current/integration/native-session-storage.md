...
| Kimi `kimi_acp` | real ACP session id captured | `~/.kimi-code/sessions/**/session_<id>/agents/main/wire.jsonl` | reviewed 0.31.0/0.31.1 prefer lightweight `session/resume`; `session/load` is a method-not-found compatibility fallback whose replay is drained before the next prompt | model/thinking selection, generation-crossing same-session resume, next-round mail, bounded full-access receipts, and cooperative `session/cancel` notification are current; live activity remains transient and native history is read on demand |
