...
--input-format stream-json`, opt-in behind `FIRM_CLAUDE_RESIDENT`, see
...
conversation hosting. It is pre-alpha and **not usable for this firm today**.
...
- It is **not** a clean blocking request/response API. `turn/start` returns
  ...
  Rust firm would need a dedicated reader thread demuxing by JSON-RPC id and
...
2. **`app-server` works but its differentiating value does not pay off here, and
   adopting it reverses a decision the firm already paid to make.** Its wins
   ...
   back-channel for mid-turn approval and steering. But (a) the firm drives
...
3. **The cost of `app-server` is high and concrete for a SYNC runtime.** It is
   ...
   fragile hand-rolled bidirectional transport class the firm already built
...
stub). app-server becomes the correct choice **if/when** the firm's
...
- **Claude: resident path added (this PR), opt-in.** Same lifecycle *shape* as
  ...
  behind `FIRM_CLAUDE_RESIDENT=1`. Default Claude behavior is unchanged.
...
> path), but is **not** promoted to default: the firm is a sync runtime,
...
```bash
cargo test -p firm-cli resident
```
