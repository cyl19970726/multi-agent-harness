...
| 模块核心思想 | 每个重要模块为什么存在、拥有什么、拒绝什么、不可破坏什么 |
...
在创建或重写文档目录前，先提炼设计依据。文档架构应该承接项目的设计思想，而不是把内容按模板填满。

最少写清楚：

```text
Design Basis
  core_thesis: 这个项目最重要的判断是什么
  layers: 系统分成哪些层，为什么这样分
  module_core_ideas: 每个重要模块为什么存在、拥有什么、拒绝什么、不能破坏什么
  relationships: 模块之间的数据流、控制流、证据流如何承接
  progression: 从整体到模块，再到子模块和实现细节如何递进
  governance: 哪些设计承诺必须由 CI/CD、schema、CLI 或 dashboard 验证
```

判断一个文档体系是否成熟，先看它能否解释这些问题：

- 为什么是这些模块，而不是别的模块。
- 为什么这样分层，层与层之间如何承接。
- 哪些模块体现系统的核心思想，哪些只是实现细节。
- 子模块如何从主模块自然深入，而不是另起一套概念。
- 重要设计依据如何映射到 docs、schemas、skills、CLI、Dashboard 和 CI/CD。

如果这个 pass 做不出来，不要急着增加目录；先继续理解项目。
...
目录架构应该根据项目实时状态生长。项目还在探索时保持扁平；模块、读者、机器消费边界稳定后再拆分；当目录结构不再表达当前系统关系时，应该 reorg。只有满足以下条件之一才拆分：
...
| 设计依据 | 解释层级、模块核心思想和关键取舍如何承接 | 只列模块，不解释为什么这样拆 |
