...
impl DeepSeekTeamRuntimeConfig {
    pub(crate) fn start_frame(&self) -> CliResult<Value> {
        ...
        .map_err(|error| {
            CliError::Usage(format!(
                "DEEPSEEK_HARNESS_PROTOCOL_ERROR: invalid embedded runner contract: {error}"
            ))
        })?;
...
pub(crate) fn validate_runner_frame(
    ...
    let name = frame
        ...
        .ok_or_else(|| {
            CliError::Usage(format!(
                "DEEPSEEK_HARNESS_PROTOCOL_ERROR: frame lacks {name_field}"
            ))
        })?;
    let schema = contract
        .get(schema_set)
        ...
        .ok_or_else(|| {
            CliError::Usage(format!(
                "DEEPSEEK_HARNESS_PROTOCOL_ERROR: contract lacks {schema_set} schema for {name}"
            ))
        })?;
    validate_schema(
        schema,
...
fn validate_schema(schema: &Value, value: &Value, path: &str) -> CliResult<()> {
    let Some(schema_object) = schema.as_object() else {
        return Err(CliError::Usage(format!(
            "DEEPSEEK_HARNESS_PROTOCOL_ERROR: invalid schema at {path}"
        )));
    };
    ...
    if let Some(items) = value.as_array() {
        let item_schema = schema.get("items").ok_or_else(|| {
            CliError::Usage(format!(
                "DEEPSEEK_HARNESS_PROTOCOL_ERROR: array schema at {path} lacks items"
            ))
        })?;
    ...
    if let Some(object) = value.as_object() {
        let properties = schema
            ...
            .ok_or_else(|| {
                CliError::Usage(format!(
                    "DEEPSEEK_HARNESS_PROTOCOL_ERROR: object schema at {path} lacks properties"
                ))
            })?;
            ...
            .filter_map(Value::as_str)
        {
            if !object.contains_key(required) {
                return Err(CliError::Usage(format!(
                    "DEEPSEEK_HARNESS_PROTOCOL_ERROR: {path} lacks required {required}"
                )));
            }
        }
        for (key, item) in object {
            if let Some(property_schema) = properties.get(key) {
                ...
                return Err(CliError::Usage(format!(
                    "DEEPSEEK_HARNESS_PROTOCOL_ERROR: {path} has unknown property {key}"
                )));
            }
        }
    }
...
fn required_contract_field<'a>(contract: &'a Value, field: &str) -> CliResult<&'a str> {
    contract
        ...
        .ok_or_else(|| {
            CliError::Usage(format!(
                "DEEPSEEK_HARNESS_PROTOCOL_ERROR: runner contract lacks {field}"
            ))
        })
}
