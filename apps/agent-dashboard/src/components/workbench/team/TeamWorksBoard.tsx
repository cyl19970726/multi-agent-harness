...
export function TeamWorksBoard({ works, members, selectedWorkId, onSelectWork, onOpenMember, onOpenHost, ownerFilter, attentionFilter, queryFilter, onFiltersChange, onOpenHostTools }: {
  ...
  return (
    <section aria-labelledby="team-works-title" data-testid="role-view-team-works">
      ...
      <div className={cn("grid gap-2 border-b border-border bg-card py-3 md:grid-cols-[minmax(18rem,1fr)_12rem_12rem]", !filtersOpen && "hidden md:grid")} aria-label="Work filters">
...
function WorkCard({work,ownerMember,selected,onSelect,register,prominent=false}:{work:WorkSummary;ownerMember?:MemberCapacitySummary;selected:boolean;onSelect:(id:string)=>void;register:(node:HTMLButtonElement|null)=>void;prominent?:boolean}) { return <button ref={register} type="button" {...(prominent ? {"data-priority-work":work.work_id} : {"data-work-card":work.work_id})} onClick={() => onSelect(work.work_id)} className={cn("agent-team-panel min-w-0 rounded-[9px] p-3.5 text-left hover:border-primary/35",selected && "agent-team-selected",prominent && "w-full bg-accent/35")}><div className="flex flex-wrap items-center justify-between gap-1"><div className="flex flex-wrap gap-1"><Badge>{work.phase}</Badge>{work.condition !== "normal" && <Badge tone={work.condition === "blocked" ? "bad" : "warn"}>{work.condition.replace(/_/g," ")}</Badge>}{work.phase === "closed" && work.resolution && <Badge tone={work.resolution === "accepted" ? "good" : work.resolution === "failed" ? "bad" : "muted"}>{work.resolution}</Badge>}</div><span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{work.priority}</span></div><h4 className="company-editorial-title mt-2.5 break-words text-[16px] leading-snug">{work.title || work.work_id}</h4><p className="mt-1.5 line-clamp-2 text-[12px] leading-relaxed text-muted-foreground">{work.completion_criteria_markdown || "No projected completion criteria."}</p><div className="mt-3.5 grid grid-cols-2 gap-2 border-b border-border pb-2.5 text-[10px] text-muted-foreground"><span className="flex items-center gap-1.5"><FileCheck2 className="size-3.5"/>{work.artifact_refs.length + work.check_refs.length} evidence refs</span><span className="flex items-center justify-end gap-1.5"><CheckCircle2 className="size-3.5"/>{work.gate_summary.passed}/{work.gate_summary.required} gates</span></div><div className="mt-2.5 flex min-w-0 items-center gap-2 text-[11px] text-muted-foreground">{ownerMember ? <><Avatar name={ownerMember.display_name} identity={`${ownerMember.agent_member_ref.id} ${ownerMember.role}`} size="xs" tone={ownerMember.runtime_state === "running" ? "running" : ownerMember.capacity === "available" ? "good" : "idle"}/><CircleDotLabel text={memberLabel(ownerMember)}/></> : <><Users className="size-3.5"/>Unassigned</>}<span className="ml-auto shrink-0 font-mono">v{work.work_revision}</span></div><div className="mt-2.5 flex items-center gap-1.5 border-t border-border pt-2.5 text-[11px] font-medium"><WorkStateMark work={work}/></div></button>; }
