...
function ExpandableEvent({data,event,onSelect}:{data:AgentWorkspaceData;event:AgentWorkspaceActivityItem;onSelect:()=>void}){
  ...
  return <article className="agent-event-row border-b border-border/65" data-kind={event.kind} data-open={open}>
    <button type="button" className="flex min-h-10 w-full items-center gap-2.5 py-2 text-left" aria-expanded={open} onClick={()=>{setOpen(value=>!value);onSelect();}}>{open?<ChevronDown className="size-3 text-muted-foreground"/>:<ChevronRight className="size-3 text-muted-foreground"/>}<span className={`agent-event-icon ${eventTone}`}><Icon className="size-[15px]"/></span><span className="aw-event-title min-w-0 flex-1 truncate">{event.title}</span><span className="aw-event-status">{humanizeToken(event.status)}</span><span className="aw-record-time w-[4.5rem] text-right">{formatTime(event.occurred_at)}</span></button>
