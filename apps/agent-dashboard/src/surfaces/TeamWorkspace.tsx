...
export function TeamWorkspace({apiUrl,space,project,teamId,teamRunId,refreshKey,selection,onAction,actionsCurrent,onSelectionChange}:{apiUrl:string;space:string;project:string;teamId:string;teamRunId?:string;refreshKey?:string;selection:SelectionState;onAction:RoleActionExecutor;actionsCurrent:boolean;onSelectionChange:(next:Partial<SelectionState>)=>void}){
  ...
  if(hostMode && team.viewer_role === "host") return <main className="h-full flex-1 overflow-y-auto p-3 sm:p-5"><div className="mx-auto max-w-[1500px]"><HostConsole embedded apiUrl={apiUrl} space={space} project={project} teamId={teamId} teamRunId={resolvedTeamRunId} selectedWorkId={selection.teamWorkId} refreshKey={refreshKey} onAction={onAction} actionsCurrent={actionsCurrent} onSelectionChange={onSelectionChange}/></div></main>;
  return <main className="h-full flex-1 overflow-y-auto p-3 sm:p-5" data-testid="authenticated-team-workspace"><div className="mx-auto max-w-[1500px] space-y-4">
    <header className="rounded-xl border border-border bg-card px-4 py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-[.15em] text-primary"><Users className="size-3.5"/>Agent Team War Room <span className="text-muted-foreground">· {team.viewer_role} view</span></div><h1 className="mt-1 break-words text-xl font-semibold sm:text-2xl">{team.display_name || team.team_id}</h1><p className="mt-1 text-xs text-muted-foreground">Mission {team.mission_id} · latest attempt {team.latest_run?.id ?? "not started"}</p></div><div className="flex flex-wrap items-center justify-end gap-2"><Button size="sm" variant="secondary" onClick={() => onSelectionChange({teamMode:"workspace",teamConversation:"host",memberRunId:undefined,teamTab:"members"})}><MessageSquare className="size-3.5"/>Host conversation</Button>{team.viewer_role === "host" && <Button size="sm" variant="secondary" aria-controls="host-tools" onClick={() => openHostTools()}><ShieldCheck className="size-3.5"/>Open Host Console</Button>}<details className="relative"><summary className="flex min-h-9 cursor-pointer list-none items-center rounded-md border border-border px-3 text-xs font-medium">Context</summary><div className="absolute right-0 top-11 z-30 w-72 rounded-xl border border-border bg-background p-4 shadow-xl"><dl className="space-y-2 text-xs"><ContextFact label="Team" value={team.team_id}/><ContextFact label="Mission" value={team.mission_id}/><ContextFact label="Team Lead" value={team.host_agent_id}/><ContextFact label="Node" value={team.node_id}/><ContextFact label="Run" value={team.latest_run?.id ?? "not started"}/><ContextFact label="Project" value={team.latest_run?.project_binding_id ?? "not attached"}/></dl><p className="mt-3 border-t border-border pt-3 text-[9px] leading-relaxed text-muted-foreground">Mission Log owns Host judgment. Provider transcripts remain native.</p></div></details><ViewProvenance view={view}/></div></div>
    ...
    {tab === "activity" && <div role="tabpanel" id="team-workspace-panel-activity" aria-labelledby="team-workspace-tab-activity" className="space-y-4"><TeamConversationStream activity={view.data.activity} messages={view.data.messages} members={view.data.members} truncated={view.data.activity_truncated} onOpenWork={(teamWorkId) => onSelectionChange({teamTab:"works",teamWorkId})} onReply={team.viewer_role === "host" ? setReplyTo : undefined}/>{team.viewer_role === "host" && <HostActivityComposer apiUrl={apiUrl} space={space} project={project} routeIdentity={teamId} teamRunId={resolvedTeamRunId} replyTo={replyTo} refreshKey={refreshKey} actionsCurrent={actionsCurrent} onAction={onAction} onClearReply={() => setReplyTo(null)}/>}</div>}
    {tab === "members" && <div role="tabpanel" id="team-workspace-panel-members" aria-labelledby="team-workspace-tab-members"><TeamMembersCapacity members={view.data.members} selectedMemberRunId={selection.memberRunId} onOpenMember={(memberRunId) => { const member=view.data.members.find((candidate) => candidate.current_member_run_ref === memberRunId); onSelectionChange({teamMode:"workspace",teamConversation:member?.agent_member_ref.id,memberRunId}); }}/></div>}
    </div>
  </div></main>;
}
