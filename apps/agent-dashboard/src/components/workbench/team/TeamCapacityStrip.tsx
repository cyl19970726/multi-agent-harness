  ...
  return (
    <section aria-label="Team capacity and pressure" data-testid="team-capacity-strip" className={cn("grid grid-cols-4 gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-5", className)}>
      {capacityFacts(summary).map((fact) => {
        ...
        return (
          <div key={fact.id} data-capacity-tile={fact.id} className={cn("min-w-0 bg-card px-2 py-2.5 sm:px-3",fact.id === "ready-members" && "hidden sm:block")}>
            ...
            <div className="mt-1 flex items-baseline gap-1"><strong className={cn("text-lg font-semibold tabular-nums sm:text-xl", pressured && "text-status-warn")}>{fact.value}</strong>{"total" in fact && <span className="text-[9px] text-muted-foreground">of {fact.total}</span>}</div>
          </div>
        );
      })}
    </section>
