...
| Agent Team | independent reusable Team identity, Mission relations, long-lived TeamRuns, run-scoped members, durable Supervisor generations, typed mail, correlations, actions and handoffs | Standing Organization membership, Wave ownership, or copied provider history | ownership is proven by assignment/message lineage; live control by the current Supervisor; execution detail by native-session bindings |
