...
export function HostConsole({apiUrl,space,project,teamId,teamRunId,refreshKey,onAction,actionsCurrent,onSelectionChange}:{apiUrl:string;space:string;project:string;teamId:string;teamRunId?:string;refreshKey?:string;onAction:RoleActionExecutor;actionsCurrent:boolean;onSelectionChange:(next:Record<string,unknown>)=>void}) {
  const [view,setView]=useState<RoleView<HostConsoleData>|null>(null);
  const [error,setError]=useState<string|null>(null);
  const [loading,setLoading]=useState(true);
  const [refresh,setRefresh]=useState(0);
  useEffect(()=>{let live=true;setLoading(true);fetchRoleView<HostConsoleData>(apiUrl,`/v1/views/host-console/${encodeURIComponent(teamId)}`,{space,project}).then(value=>{if(live){setView(value);setError(null)}}).catch(reason=>live&&setError(String(reason))).finally(()=>live&&setLoading(false));return()=>{live=false}},[apiUrl,space,project,teamId,refreshKey,refresh]);
