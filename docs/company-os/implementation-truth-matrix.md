...
| Organization | `organization-and-actors.md`; Human, Standing Agent, External, Service, OrgUnit and Membership; optional `company-org-operator` skill | `schemas/company-os/actors.schema.json` plus optional `MemberRun.agent_member_id` in `schemas/member-run.schema.json` | typed actor and organization ledgers with reference validation; Agent Team participation remains in execution ledgers and is joined read-only | resource reads plus flat `harness company org list/query/create-human/create-agent/create-unit/add-membership/transition-actor/update-permissions` and nested `harness company org actor/unit/membership ...` administrative authoring; Company snapshot derives explicit `standing_assignments`; no governed Org/HR lifecycle proposal/approval Action family yet | Store-live organization plus a shared FocusShell Standing Agent workspace show identity, reporting projection, configuration refs, WorkItems, explicitly linked Agent Team assignments and durable activity; Organization cards and Agent Team participation deep-link to native detail surfaces | core/store/API reference tests, Mission/Team identity-link acceptance, Dashboard type/build checks, navigation checks, `check-company-os-org-cli-smoke`, and operator CLI smoke | **partial; Organization CLI v1, Standing Agent profile, and explicit Agent Team participation projection verified** — native prompt/tool/skill/Docs/WorkType/escalation refs, OrgUnit membership, declared status, permission/capability ref maintenance, stable StandingAgent/AgentMember/MemberRun identity join, and distinct execution boundary exist; availability/capacity truth, Workflow/direct durable-agent participation, durable cross-client Supervisor health, and governed organization evolution proposal/approval do not |
...
| Finance | `financial-relations.md`; Commitment and Payment stay separate; optional `company-finance-operator` skill | `schemas/company-os/finance.schema.json` | separate Commitment and Payment ledgers with monotonic validation | flat `harness company finance list/query/propose-commitment/request-approval/decide-approval/transition-commitment/record-payment/transition-payment`, nested `harness company finance commitment/payment ...`, shared `harness company approval ...`, and evidence-backed prepared Payment records; the flat initial proposed Commitment path remains Human administrative in v1 while nested operator flow uses the governed Action dispatcher where policy context exists; budget/invoice/refund/reporting and deeper settlement transition remain planned | Finance and Approval views show Store-live monetary state and explicitly distinguish commitment from payment | core/store/API financial boundary tests, browser approval checks, `check-company-os-finance-cli-smoke`, and operator CLI smoke | **partial overall; verified for native Commitment, approval, and prepared Payment CLI flow** — proposed Commitment creation is native Store truth; no Payment is inferred from an approved Commitment, and Payment requires separate approval/evidence |
