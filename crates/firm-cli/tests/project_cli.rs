...
//! developer's real `~/.firm` is never touched. The same registry/marker code is
...
fn current_id(home: &TempHome, cwd: &Path) -> Option<String> {
    let out = run_firm(home, cwd, &["project", "current"]);
...
fn add_registers_without_switching_then_switch_makes_current() {
    ...
    let out = run_firm(&home, &repo_a, &["project", "add"]);
    ...
    let listed = json_out(&run_firm(&home, home.base(), &["project", "list"]));
    ...
    let out = run_firm(&home, &repo_b, &["project", "add", "--switch"]);
...
fn switch_updates_registry_and_active_marker_consistently() {
    ...
    let out = run_firm(&home, home.base(), &["project", "switch", "_global"]);
    ...
    let out = run_firm(&home, home.base(), &["project", "switch", "no-such-id"]);
...
fn remove_refuses_current_without_force_and_protects_global() {
    ...
    let out = run_firm(&home, home.base(), &["project", "remove", "_global"]);
...
fn remove_unknown_id_is_an_error() {
    ...
    let out = run_firm(&home, home.base(), &["project", "remove", "ghost"]);
...
fn subcommands_are_documented_in_help() {
    ...
    let out = run_firm(&home, home.base(), &["help"]);
