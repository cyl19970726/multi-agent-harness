...
> **Historical compatibility model.** This describes implemented GoalPhase and
> Task Graph behavior. New execution design uses lightweight Mission/Wave as
> defined by [ADR 0026](../../decisions/0026-mission-wave-architecture.md); new company
> work uses the [Company OS WorkItem contract](../../company-os/work-items-and-approvals.md).
> Do not derive new Wave or WorkItem requirements from this document.
...
2. Reuses an in-flight `GoalOrchestrationRun` with `status == Running` as a resume checkpoint, else starts a fresh one (`main.rs:2254-2273`) — this reuse is **unconditional**: the `--resume` flag is an auditable opt-in marker (parsed at `main.rs:3211`), not a behavior toggle; the orchestrator re-enters the checkpoint with or without it. `GoalOrchestrationRun` (`lib.rs:301`) persists `phase_runs: Vec<OrchestrationPhaseRun>` (`lib.rs:280`), and each `OrchestrationPhaseRun` carries a `workflow_run_id` — the BACK link by which a goal reaches its workflow runs — so completed phases are not re-spent. Stage 0 added the FORWARD link too: every orchestrated `WorkflowRun` carries `goal_id`/`phase_id` (stamped by `run_phase_compiled_script` via `RunGoalLink`), so a run is reverse-indexable to its goal/phase without parsing `workflow_name`.
