...
baseline: master 35bcda73; every claim below about today's code was verified against that
        exact revision in a read-only checkout
```
...
(`store_node_runtime.rs:273-334`, `:361-420`, `:424-466`, `:468-520`) — the same lock that
...
- **The heartbeat waits in that same queue, with a budget equal to its own remaining
  ...
  `budget = remaining TTL` (`store_node_runtime.rs:375-386`), then queues on `.store.lock`.
...
- **Every renewal also rewrites the lease ledger.** `compact_node_daemon_leases_unlocked` runs
  on each renewal (`store_node_runtime.rs:404`, `store_jsonl.rs:195-252`): full read + temp +
...
`AGENTS.md`'s "never scoped to one Execution Space" is not true of the rows. Second, **readers
already read it lock-free**: the admission fence (`fabric_foundation.rs:34-92`), the settlement
fence (`:94-…`), `from_admitted_command` (`provider_capabilities.rs:453-462`),
`current_node_daemon_lease_after_admission_at` (`runtime_command_admission.rs:9-31`) and the
three TeamSupervisorLease parent fences (`store_node_runtime.rs:121-142`, `:552-576`,
`:687-712`) all resolve through `latest_node_daemon_lease`, an unsynchronized `read_jsonl`
(`store_read_models.rs:325-331`; the torn-tail retry that makes it survivable is documented at
`store_jsonl.rs:322-345`). The write lock is not buying reader consistency; it is buying a
queue — while every lease row in evidence is **351–365 bytes** (median 353).
...
carry a pre-lock timestamp past expiry (`store_node_runtime.rs:100-104`, with `renewal_now()`
re-sampled at `:393` and again after compaction at `:409`). The equivalent guarantee here:
...
- **`rename` + `fsync` semantics.** Atomic only within one filesystem — the temp file sits in
  ...
  `store_jsonl.rs:243-250`; on macOS/APFS this is the primitive the trust journal already relies
