...
> Work-graph amendment (ADR 0058): this historical ADR's “no Task Graph” clause
> excludes a second universal task ledger or executor, not the current flat Work
> dependency DAG. References to Harness-created child Team lifecycle are
> historical; Teams and Works are flat, and Work ordering uses explicit
> `depends_on` edges.
