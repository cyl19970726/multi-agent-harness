...
pub(crate) fn team_view(
    ...
    let host_runtime = run
        ...
        .transpose()?;
    // ADR 0075: the daemon summary shows who owns this machine, so it asks the
    // record that decides it. Resolved once rather than twice, and through
    // `current_machine_lease` so a pre-cutover Store still renders — the source
    // travels with the lease and a legacy row is shown, never acted on.
    let machine_lease = store
        .current_machine_lease(&team.node_id)
        .ok()
        .flatten()
        .map(|(lease, _)| lease);
    Ok(envelope(
