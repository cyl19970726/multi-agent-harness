...
fn member_inbox_is_latest_wins_and_defaults_to_actionable_mail() {
    ...
    let actionable = team_run_inbox(&store, &created.team_run.id, &first.id, false).expect("inbox");
    ...
    let history = team_run_inbox(&store, &created.team_run.id, &first.id, true).expect("all inbox");
