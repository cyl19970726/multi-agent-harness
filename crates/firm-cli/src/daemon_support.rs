...
pub(crate) fn latest_team_run(store: &HarnessStore, id: &str) -> CliResult<AgentTeamRun> {
    store
        .latest_team_runs()?
        .into_iter()
        .find(|run| run.id == id)
...
pub(crate) fn is_unclosed_managed_member(
...
pub(crate) const CANONICAL_STATE_EVIDENCE_PREFIX: &str = "team-run-canonical-state:";
pub(crate) fn canonical_state_evidence_ref(fingerprint: &str) -> String {
    format!("{CANONICAL_STATE_EVIDENCE_PREFIX}{fingerprint}")
}
pub(crate) fn canonical_state_from_evidence(evidence_refs: &[String]) -> Option<&str> {
    evidence_refs
        .iter()
...
pub(crate) fn current_unix_ms_u64() -> u64 {
