//! Proves the workflow completion hook (`FIRM_WORKFLOW_ON_COMPLETE`) fires when
...
fn init_store(store: &Path) {
    let out = harness()
        ...
        .env_remove("FIRM_WORKFLOW_ON_COMPLETE")
        .output()
        .expect("run init");
    assert!(
        out.status.success(),
...
fn completion_hook_fires_with_run_id_and_status_and_is_noop_without_env() {
    ...
    assert!(
        !noenv_marker.exists(),
        "completion hook ran with no FIRM_WORKFLOW_ON_COMPLETE set — it must be a no-op"
    );
    ...
    let out = harness()
        ...
        .env_remove("FIRM_ROOT")
        .env("FIRM_WORKFLOW_ON_COMPLETE", &hook_cmd)
        .output()
