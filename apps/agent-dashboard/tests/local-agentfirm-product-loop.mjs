...
for(const surface of ["CompanyWorkIndex","TeamWorkspace","HostConsole","MemberWorkbench","OperatorView"])assert.ok(fs.existsSync(`apps/agent-dashboard/src/surfaces/${surface}.tsx`),`${surface} missing`);
for(const marker of ["<CompanyWorkIndex","<TeamWorkspace","<MemberWorkbench","<OperatorView"])assert.ok(shell.includes(marker),`route missing ${marker}`);
...
const combined=["CompanyWorkIndex","TeamWorkspace","HostConsole","MemberWorkbench","OperatorView","RoleViewPrimitives"].map(name=>fs.readFileSync(`apps/agent-dashboard/src/surfaces/${name}.tsx`,"utf8")).join("\n");
