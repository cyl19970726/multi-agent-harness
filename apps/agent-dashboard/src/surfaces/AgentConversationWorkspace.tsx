...
export function AgentConversationWorkspace({
  ...
  return <Tooltip.Provider delayDuration={350}>
    <main className="agent-team-surface agent-workspace h-full min-h-0 flex-1 overflow-hidden" data-testid="agent-workspace">
      <div className="agent-workspace-layout grid h-full min-h-0 grid-cols-1 lg:grid-cols-[14.5rem_minmax(42rem,1fr)_17.5rem]">
        ...
        <section className="agent-workspace-center flex min-h-0 min-w-0 flex-col">
          <header className="agent-workspace-header flex min-h-[5.5rem] shrink-0 items-center gap-3 border-b border-border px-4 sm:px-7">
          ...
          <Tabs.Root value={mode} onValueChange={value=>{setContextSelection(null);onSelectionChange({agentWorkspaceMode:value as WorkspaceMode});}} className="flex min-h-0 flex-1 flex-col">
            <div className="aw-modebar flex min-h-12 shrink-0 items-end border-b border-border px-4 sm:px-7">
...
function AuthoredTurn({data,message,selectedAgentId,onSelect}:{data:AgentWorkspaceData;message:MessageSummary;selectedAgentId:string;onSelect:()=>void}){
  ...
  return <article role="button" tabIndex={0} aria-label={`Open authored Message from ${name}`} data-from-selected={fromSelected||undefined} className="agent-authored-turn" onClick={onSelect} onKeyDown={event=>activateOnKeyDown(event,onSelect)} onKeyUp={event=>activateOnKeyUp(event,onSelect)}>
    ...
    <div className="aw-record-meta">{message.work_id&&<span>Work · {shortId(message.work_id)}</span>}{message.correlation_id&&<span>Correlation · {shortId(message.correlation_id)}</span>}{message.deliveries.length>0&&<span>Delivery · {message.deliveries.map(delivery=>delivery.status).join(" · ")}</span>}</div></div>
  </article>;
}
...
function ExpandableEvent({data,event,onSelect}:{data:AgentWorkspaceData;event:AgentWorkspaceActivityItem;onSelect:()=>void}){
  ...
  if(authored)return <article role="button" tabIndex={0} aria-label={`Open provider-authored reply from ${data.selected_agent.display_name}`} className="agent-authored-turn" onClick={onSelect} onKeyDown={keyEvent=>activateOnKeyDown(keyEvent,onSelect)} onKeyUp={keyEvent=>activateOnKeyUp(keyEvent,onSelect)}><Avatar name={data.selected_agent.display_name} identity={`${data.selected_agent.agent_member_ref.id} ${data.selected_agent.role}`} size="md" tone={data.selected_agent.runtime_status==="running"?"running":"idle"}/><div className="min-w-0 flex-1"><header className="mb-1 flex items-baseline gap-2"><p className="text-[12px] font-semibold">{data.selected_agent.display_name}</p><span className="aw-record-kind">Provider reply</span><time className="ml-auto text-[10px] text-muted-foreground">{formatTime(event.occurred_at)}</time></header><div className="text-[13px] leading-[1.62]"><Markdown source={event.summary??"Provider recorded an authored response."}/></div></div></article>;
  return <article className="agent-event-row border-b border-border/65" data-kind={event.kind} data-open={open}>
    <button type="button" className="flex min-h-10 w-full items-center gap-2.5 py-2 text-left" aria-expanded={open} onClick={()=>{setOpen(value=>!value);onSelect();}}>{open?<ChevronDown className="size-3 text-muted-foreground"/>:<ChevronRight className="size-3 text-muted-foreground"/>}<span className={`agent-event-icon ${eventTone}`}><Icon className="size-[15px]"/></span><span className="min-w-0 flex-1 truncate text-[11px] font-medium">{event.title}</span><span className="text-[10.5px] text-muted-foreground">{event.status}</span><span className="w-[4.5rem] text-right text-[10.5px] text-muted-foreground">{formatTime(event.occurred_at)}</span></button>
...
function MessagesCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(next:ContextSelection)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="agent-messages-canvas mx-auto max-w-[58rem] px-5 pb-10 sm:px-7">
    ...
    {visible.length?<div className="agent-message-stream">{visible.map(message=>{const actor=data.roster.find(item=>item.agent_member_ref.id===message.sender.id);const actorName=actor?.display_name??message.sender.id;const recipients=message.recipients.map(recipient=>data.roster.find(item=>item.agent_member_ref.id===recipient.id)?.display_name??recipient.id).join(", ");return <button key={message.message_id} type="button" className="agent-message-row flex w-full gap-3 text-left" onClick={()=>onSelect({kind:"message",message})}><Avatar name={actorName} identity={`${message.sender.id} ${actor?.role??""}`} size="md" tone={actor?.runtime_state==="running"?"running":"idle"}/><span className="min-w-0 flex-1"><span className="flex items-baseline gap-2"><b className="truncate text-[12px]">{actorName}</b><span className="aw-record-kind">{message.sender.id===selectedId?"Outbox":"Inbox"} → {recipients}</span><time className="ml-auto text-[10.5px] text-muted-foreground">{formatTime(message.created_at)}</time></span><span className="mt-1 block max-w-[42rem] whitespace-pre-wrap text-[13px] leading-[1.55] text-foreground/90">{message.body}</span><span className="aw-record-meta">{message.work_id&&<span>Work · {shortId(message.work_id)}</span>}<span>{message.deliveries.map(delivery=>delivery.status).join(", ")||"recorded"}</span></span></span></button>})}</div>:<EmptyCanvas title="No matching messages" detail="Inbox and Outbox stay in Harness coordination records; provider-private transcripts are not mirrored here."/>}
...
function WorkCanvas({data,onSelect}:{data:AgentWorkspaceData;onSelect:(work:WorkSummary)=>void}){
  ...
  return <ScrollArea.Root className="h-full overflow-hidden"><ScrollArea.Viewport className="size-full"><div className="agent-work-canvas mx-auto max-w-[60rem] px-5 pb-10 sm:px-7">
    ...
    {visible.length?<div className="agent-work-stream">{visible.map((work,index)=><button key={work.work_id} type="button" data-current={lens==="current"&&index===0||undefined} className="agent-work-row grid w-full grid-cols-[minmax(0,1fr)_7rem_7rem] gap-4 text-left" onClick={()=>onSelect(work)}><span className="min-w-0"><span className="flex items-center gap-2"><span className="truncate text-[13px] font-semibold">{work.title||work.work_id}</span>{work.condition!=="normal"&&<WorkspaceState label={work.condition} tone="bad"/>}</span><span className="aw-record-meta"><span>{shortId(work.work_id)} · v{work.work_revision}</span><span>{work.priority}</span></span><span className="mt-2 line-clamp-2 text-[12px] leading-5 text-foreground/75">{work.completion_criteria_markdown}</span></span><span><span className="agent-team-phase-label">{work.phase}</span><span className="mt-2 block text-[10.5px] text-muted-foreground">{work.delivery_summary.recovery_class||"delivery clear"}</span></span><span className="text-right"><span className="text-[10.5px] font-semibold">{work.gate_summary.passed}/{work.gate_summary.required} gates</span><span className="mt-2 block text-[10.5px] text-muted-foreground">{formatTime(work.updated_at)}</span></span></button>)}</div>:<EmptyCanvas title="No Work in this view" detail="Eligibility is not ownership. Work remains authoritative in the Team Work kernel."/>}
  </div></ScrollArea.Viewport></ScrollArea.Root>;
}
...
function AgentContextRail({view,data,selected,currentWork,actions}:{view:RoleView<AgentWorkspaceData>;data:AgentWorkspaceData;selected:ContextSelection;currentWork?:WorkSummary;actions:AllowedAction[]}){
  ...
  return <ScrollArea.Root className="h-full min-w-0 overflow-hidden"><ScrollArea.Viewport className="size-full min-w-0 [&>div]:!block [&>div]:!min-w-0"><div className="min-w-0 overflow-hidden px-5 pb-8 pt-5">
    ...
    <ContextSection title={isHost?"Available Host Controls":"Available Controls"}><WorkspaceActionIndex label={isHost?"Available Host Controls":"Available Controls"} actions={actionIndex.slice(0,6).map(action=>({key:action.kind,label:actionLabel(action.kind),disabledReason:action.disabled_reason}))}/></ContextSection>
...
function WorkContext({title="Current Work",work}:{title?:string;work?:WorkSummary}){return <ContextSection title={title} primary>{work?<><h3 className="text-[14px] font-semibold leading-5">{work.title||work.work_id}</h3><div className="mt-2 flex items-center gap-2"><WorkspaceState label={work.phase} tone={work.phase==="active"?"running":work.phase==="review"?"warn":work.phase==="closed"?"good":"muted"}/><span className="text-[10px] text-muted-foreground">{shortId(work.work_id)} · v{work.work_revision}</span></div><div className="mt-4"><ContextFact label="Condition" value={work.condition}/><ContextFact label="Priority" value={work.priority}/><ContextFact label="Delivery" value={String(work.delivery_summary.recovery_class??"clear")}/><ContextFact label="Gates" value={`${work.gate_summary.passed}/${work.gate_summary.required}`}/></div></>:<p className="text-[11px] leading-5 text-muted-foreground">No Work is currently owned or eligible for this Agent.</p>}</ContextSection>}
...
function ContextFact({label,value}:{label:string;value:string}){return <WorkspaceFact label={label} value={value}/>}
...
function ContextWorkRow({work,evidence=false}:{work:WorkSummary;evidence?:boolean}){return <div className="border-t border-border/70 py-2.5 first:border-t-0"><div className="flex items-start justify-between gap-3"><p className="min-w-0 flex-1 truncate text-[11.5px] font-semibold">{work.title||work.work_id}</p><span className="shrink-0 text-[10.5px] text-muted-foreground">{evidence?(work.latest_report_ref?"report":work.latest_finding_refs.length?"finding":work.latest_failure_ref?"failure":"evidence"):work.phase}</span></div><p className="mt-1 truncate text-[10.5px] text-muted-foreground">{shortId(work.work_id)} · {work.owner_actor_ref?.id??"unassigned"}</p></div>}
...
function TagList({values,empty}:{values:string[];empty:string}){return values.length?<div className="flex flex-wrap gap-2">{values.map(value=><span key={value} className="rounded-md bg-secondary px-2 py-1 text-[11px]">{value}</span>)}</div>:<p className="text-[11px] text-muted-foreground">{empty}</p>}
function ProfileList({label,values,empty}:{label:string;values:string[];empty:string}){return <div className="mt-3 first:mt-0"><p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{label}</p><TagList values={values} empty={empty}/></div>}
