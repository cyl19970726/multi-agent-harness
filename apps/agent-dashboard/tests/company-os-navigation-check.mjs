...
async function main() {
  ...
  check(api.includes("...options.headers") && api.includes("payload.detail || payload.error") && app.includes("postAction(apiUrl, path, body, selectedProjectId, selectedCompanyId, options)"), "browser Action requests carry scoped headers, preserve server denial detail, and refresh through the existing mutation path");
