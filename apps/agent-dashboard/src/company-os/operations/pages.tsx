...
import { prototypeTrademarkOperationsProjection, standingExecutionAssignmentsForActor } from "./fixture";
...
function OrgActorCard({ view, actor, membership, variant = "member", className, onOpen }: { view: TrademarkOperationsProjection; actor: ActorSummary; membership?: OrganizationMembershipView; variant?: "owner" | "lead" | "member" | "external"; className?: string; onOpen?: () => void }) {
  ...
  const assignments = standingExecutionAssignmentsForActor(actor, view.standingAssignments);
  return <article data-company-os-ref={membership?.id ?? actor.id} data-organization-actor-ref={actor.id} data-actor-kind={actorKind} data-actor-type={actorKind} className={`${variant === "lead" ? "border-status-good/45 bg-status-good/[0.045]" : variant === "external" ? "border-sky-500/35 bg-sky-500/[0.035]" : proposed ? "border-primary/45 border-dashed bg-primary/[0.035]" : "border-border bg-card/90"} min-w-0 rounded-xl border p-4 shadow-sm ${className ?? ""}`}>{onOpen ? <button type="button" onClick={onOpen} className="w-full min-w-0 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><OrgActorCardBody actor={actor} variant={variant} proposed={proposed} /></button> : <OrgActorCardBody actor={actor} variant={variant} proposed={proposed} />}{membership && <div className="mt-3 min-w-0 border-t border-border pt-2 text-[10px] text-muted-foreground"><p className="break-words">{membership.membershipRole ?? "role not supplied"}{membership.titleOrFunction ? ` · ${membership.titleOrFunction}` : ""}{membership.status ? ` · ${membership.status}` : ""}</p><code className="mt-1 block break-all font-mono">{membership.id}</code>{membership.authorityPolicyRefs.length > 0 && <p className="mt-1 break-words">Declared membership policies: {membership.authorityPolicyRefs.join(", ")}</p>}</div>}{actor.kind === "standing_agent" && <div className="mt-3 min-w-0 rounded-lg border border-border/70 bg-background/55 p-2 text-[10px]"><p className="font-semibold text-foreground">AgentMember / runtime binding</p>{actor.executionAgentMemberRef ? <><code className="mt-1 block break-all font-mono text-muted-foreground">{actor.executionAgentMemberRef}</code>{assignments.length > 0 ? <div className="mt-2 space-y-1">{assignments.map((assignment) => <div key={assignment.id} data-company-os-ref={assignment.memberRunId} data-runtime-availability={assignment.nativeSession?.availability ?? "not reported"} className="min-w-0 rounded-md border border-border/50 bg-background/60 p-1.5 text-muted-foreground"><p className="break-words">MemberRun {assignment.status} · native session {assignment.nativeSession?.availability ?? "availability not reported"}</p><code className="mt-0.5 block break-all font-mono">{assignment.memberRunId}</code></div>)}</div> : <p className="mt-1 break-words text-muted-foreground">No MemberRun in this execution snapshot explicitly binds that AgentMember.</p>}</> : <p className="mt-1 break-words text-muted-foreground">No execution_agent_member_ref is recorded.</p>}</div>}</article>;
}

function OrgActorCardBody({ actor, variant, proposed }: { actor: ActorSummary; variant: "owner" | "lead" | "member" | "external"; proposed: boolean }) {
  return <><div className="flex min-w-0 items-start gap-3"><ActorAvatar identity={`${actor.id} ${actor.role}`} name={actor.name} size={variant === "lead" || variant === "owner" ? "lg" : "md"} ring={variant === "owner" ? "warm" : variant === "lead" ? "good" : variant === "external" ? "external" : "neutral"} /><div className="min-w-0 flex-1"><div className="flex min-w-0 flex-wrap items-center gap-2"><h3 className={`${variant === "lead" || variant === "owner" ? "company-editorial-title text-xl" : "text-sm font-semibold"} min-w-0 break-words`}>{actor.name}</h3>{actor.availability === "available" && <span className="size-2 shrink-0 rounded-full bg-status-good" title="Explicitly reported available" />}{proposed && <StatusTag status="proposed" />}</div><p className="mt-1 break-words text-xs text-muted-foreground">{actor.kind === "human" ? "Human" : actor.kind === "external" ? "External Collaborator" : "Standing Agent"} · {actor.role}</p></div></div>{variant === "lead" && <div className="mt-4 grid grid-cols-3 divide-x divide-border rounded-lg border border-border bg-background/60 py-3 text-center text-xs"><div><strong className="block break-words text-base">{actor.availability ?? "Not reported"}</strong><span className="text-muted-foreground">Declared availability</span></div><div><strong className="block break-words text-base">{actor.organizationRoleState ?? "Not reported"}</strong><span className="text-muted-foreground">Organization status</span></div><div><strong className="block text-base">Lead</strong><span className="text-muted-foreground">Explicit unit ref</span></div></div>}</>;
}
...
export function StandingAgentFocus({ data, actorId, onSelectionChange }: OperationsPageProps & { actorId?: string; onSelectionChange?: (selection: Partial<SelectionState>) => void }) {
  ...
  const ledUnitMembers = leadUnits
  ...
  const memberUnitAgentLeads = membershipUnits
    .map((unit) => unit.agentLeadActorId ? view.actors[unit.agentLeadActorId] : undefined)
    .filter((candidate, index, all): candidate is ActorSummary => {
      if (!candidate || candidate.id === actor.id) return false;
  ...
  const linkedExecutionAssignments = standingExecutionAssignmentsForActor(actor, view.standingAssignments);
  ...
  const configurationEmpty = !actor.systemPromptRef
    ...
    && !(actor.capabilityRefs?.length)
    && !(actor.runtimeRefs?.length)
    && !(actor.nativeSessionRefs?.length);
  const membershipAuthorityPolicyRefs = [...new Set(memberships.flatMap((membership) => membership.authorityPolicyRefs))];
    ...
    <FocusShell
      ...
      context={<WorkbenchContextRail label="Organization context" quiet>
        <ContextModule title="Organization identity" kicker={`${memberships.length} explicit memberships`} icon={<Bot className="size-3.5" />} tone={actor.availability === "available" ? "good" : undefined}>
          <dl className="space-y-2 text-xs"><RailFact label="Units" value={membershipUnits.map((unit) => unit.label).join(", ") || "Not linked"} /><RailFact label="Declared availability" value={actor.availability ?? "Not reported"} /><RailFact label="Declared capacity" value={actor.assignmentCapacity ? `${actor.assignmentCapacity}` : "Not reported"} /><RailFact label="Visible workload" value={assignedWork ? "Active assignment visible" : "No linked active assignment"} /></dl>
          {memberUnitAgentLeads.length > 0 && <div className="mt-3 border-t border-border/70 pt-3"><p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Explicit Agent leads for member units</p><div className="space-y-2">{memberUnitAgentLeads.map((lead) => <ActorPill key={lead.id} actor={lead} compact />)}</div></div>}
          {isLead && ledUnitMembers.length > 0 && <div className="mt-3 border-t border-border/70 pt-3"><p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Members in explicitly led units</p><p className="mb-2 text-[10px] leading-4 text-muted-foreground">Membership does not create a ReportingRelation.</p><div className="space-y-2">{ledUnitMembers.map((member) => <ActorPill key={member.id} actor={member} compact />)}</div></div>}
        </ContextModule>
        <ContextModule title="Runtime availability" kicker="Exact execution join only" icon={<Clock3 className="size-3.5" />} collapsible defaultOpen={linkedExecutionAssignments.length > 0}>
          <p className="mb-2 text-xs leading-5 text-muted-foreground">Runtime state never changes declared Organization availability or effective authority.</p>
          {actor.executionAgentMemberRef ? <ReferenceGroup label="AgentMember" values={[actor.executionAgentMemberRef]} /> : <p className="text-xs text-muted-foreground">No execution_agent_member_ref is recorded.</p>}
          {recentExecutionAssignments.length > 0 ? <div className="mt-2 space-y-2">{recentExecutionAssignments.map((assignment) => <div key={assignment.id} data-runtime-availability={assignment.nativeSession?.availability ?? "not reported"} data-native-session-id={assignment.nativeSession?.native_session_id ?? ""} className="rounded-lg border border-border bg-background/70 p-2 text-xs"><RailFact label="MemberRun" value={`${assignment.memberRunId} · ${humanReadable(assignment.status, assignment.status)}`} /><RailFact label="Native session" value={assignment.nativeSession?.native_session_id ?? "Not linked"} /><RailFact label="Provider / mode" value={[assignment.nativeSession?.provider, assignment.nativeSession?.execution_mode].filter(Boolean).join(" · ") || "Not reported"} /><RailFact label="Runtime availability" value={assignment.nativeSession?.availability ?? "Not reported"} /></div>)}</div> : <p className="mt-2 text-xs leading-5 text-muted-foreground">No MemberRun in this execution snapshot explicitly binds that AgentMember.</p>}
        </ContextModule>
        <ContextModule title="Agent Team participation" kicker="Explicit identity links only" icon={<Network className="size-3.5" />} collapsible defaultOpen={linkedExecutionAssignments.length > 0}>
          {linkedExecutionAssignments.length > 0 ? <div className="space-y-2"><p className="text-[10px] text-muted-foreground">Showing {recentExecutionAssignments.length} of {linkedExecutionAssignments.length} chronological records.</p>{recentExecutionAssignments.map((assignment) => <button key={assignment.id} type="button" onClick={onSelectionChange ? () => onSelectionChange({ surface: "team", teamId: assignment.teamRunId, memberRunId: assignment.memberRunId, missionId: assignment.missionId, waveId: assignment.waveId }) : undefined} className="w-full rounded-lg border border-border bg-background/70 p-2 text-left transition hover:border-primary/35 hover:bg-primary/[0.035] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className="block text-xs font-semibold">{assignment.role}</span><span className="mt-1 block text-[10px] text-muted-foreground">{humanReadable(assignment.status, assignment.status)} · TeamRun {assignment.teamRunId}</span><code className="mt-1 block break-all font-mono text-[9px] text-muted-foreground">{assignment.memberRunId}</code></button>)}</div> : <p className="text-xs leading-5 text-muted-foreground">No MemberRun explicitly links this durable identity. Similar names or providers are not joined.</p>}
        </ContextModule>
        <ContextModule title="Declared permission configuration" kicker="Policy references, not evaluated grants" icon={<KeyRound className="size-3.5" />} collapsible>
          <ReferenceGroup label="Actor permission policies" values={actor.permissionPolicyRefs} />
          <ReferenceGroup label="Membership authority policies" values={membershipAuthorityPolicyRefs} />
        </ContextModule>
        <ContextModule title="Effective delegated authority" kicker="Target projection not implemented" icon={<ShieldCheck className="size-3.5" />}>
          <div data-effective-authority-status={view.organization.effectiveDelegatedAuthority.status} className="text-xs leading-5 text-muted-foreground">{view.organization.effectiveDelegatedAuthority.detail}</div>
        </ContextModule>
        <ContextModule title="Prompt, tools & skills" kicker="Configuration references" icon={<Wrench className="size-3.5" />} collapsible defaultOpen={!configurationEmpty}>
          ...
          <ReferenceGroup label="Capabilities" values={actor.capabilityRefs} />
          <ReferenceGroup label="Declared runtime locators" values={actor.runtimeRefs} />
          <ReferenceGroup label="Declared native-session locators" values={actor.nativeSessionRefs} />
        ...
        <ContextModule title="Implemented vs target" icon={<Scale className="size-3.5" />}>
          <p className="text-xs leading-5 text-muted-foreground">Implemented: declared Organization identity, membership, policy/configuration refs, Work responsibility, and exact execution links. Target: evaluated scoped grants and effective delegated authority. Tools, Skills, runtime health, and declared policy refs never grant authority by themselves.</p>
        </ContextModule>
