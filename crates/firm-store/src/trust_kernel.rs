...
impl HarnessStore {
    ...
    fn commit_trust_projection_unlocked<T: Serialize + for<'de> Deserialize<'de> + Clone>(
        ...
        if crate::wake_relevant_commit(aggregate_kind, transition) {
            crate::bump_wake_hint(self.root(), aggregate_kind);
        }
...
mod tests {
    ...
    #[test]
    fn wake_relevant_commits_bump_wake_hint_but_claims_do_not() {
        let (store, root) = fabric_store();
        for id in ["sender", "recipient"] {
            store
                .create_agent_identity(
                    &context("host", "identity.create", &format!("hint-identity-{id}"), 0),
                    identity(id),
                )
                .unwrap();
        }
        for (session_id, identity_id) in [
            ...
            store
                .create_agent_session(
                    &service_context("session.create", &format!("hint-{session_id}"), 0),
        ...
        let hint_path = crate::wake_hint_path(&root);
        // The subscription commit itself is not wake-relevant.
        assert!(
            !hint_path.exists(),
            "non-wake commits must not create the hint file"
        );
        ...
        let author = |id: &str| {
            ...
            let body_digest = format!("sha256:{:x}", Sha256::digest(id.as_bytes()));
            let fingerprint = canonical_json_fingerprint(&serde_json::json!({
                "sender_actor_ref": {"kind": "agent_member", "id": "sender"},
            ...
            store
                .author_message(
                    &service_context("message.author", id, 0),
                    Message {
                        id: id.into(),
                        source_execution_space_id: "space-test".into(),
                        source_node_id: "11111111-1111-4111-8111-111111111111".into(),
        ...
        author("message-hint-1");
        let after_first =
            fs::read_to_string(&hint_path).expect("authoring a message must bump the wake hint");
        assert!(after_first.contains("\"source\":\"message\""));

        let delivery = store.fabric_message_deliveries("space-test").unwrap();
        store
            .claim_message_for_provider(
                &service_context("message.claim", "hint-claim-1", 0),
                &delivery[0].id,
                "11111111-1111-4111-8111-111111111111",
                "daemon-1",
                1,
            ...
            .unwrap();
        let after_claim = fs::read_to_string(&hint_path).unwrap();
        assert_eq!(
            after_first, after_claim,
            "read-side claim bookkeeping must not bump the wake hint"
        );

        author("message-hint-2");
        let after_second = fs::read_to_string(&hint_path).unwrap();
        assert_ne!(
            after_first, after_second,
            "each wake-relevant commit must change the hint content"
        );
